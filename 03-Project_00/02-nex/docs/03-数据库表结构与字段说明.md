...
**索引**：`idx_user`(user_id), `idx_action`(action), `idx_created`(create_time)
...
**索引**：`idx_user`(user_id), `idx_read`(is_read), `idx_created`(create_time)
...
| remark | text | | NULL | 备注 |
| create_time | datetime | NOT NULL | CURRENT_TIMESTAMP | 更换时间 |

**索引**：`idx_part_id`(part_id), `idx_template_key`(template_key), `idx_create_time`(create_time)
...
- **时间字段**：`create_time`/`update_time` 或 `create_time`/`update_time`（⚠️两种命名混用，建议统一）
...
1. **时间字段命名不统一**：部分表使用 `create_time`/`update_time`，部分使用 `create_time`/`update_time`，建议统一为 `create_time`/`update_time`。
...
5. **审计日志无更新时间**：nex_audit_log 只有 create_time，无 update_time（审计日志不可修改，设计合理）。
