...
export function generateOrderReport(order, config = {}, t = (key) => key) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      title.textContent = t('production.order.report.title') || '订单生产报告'
      ...
      subtitle.textContent = `${t('production.order.report.generatedBy') || '报告生成人'}：${exporter}  |  ${t('production.order.report.generatedAt') || '报告生成时间'}：${timeStr}  |  ${t('production.order.page.orderNo') || '订单编号'}：${order.orderNo}`
      ...
      basicTitle.textContent = t('production.order.report.basicInfo') || '基本信息'
      ...
      const basicData = [
        [t('production.order.page.orderNo') || '订单编号', order.orderNo, t('production.order.page.productName') || '产品名称', order.productName],
        [t('production.order.page.recipeName') || '配方', order.recipeName, t('production.order.page.batchNo') || '批次号', order.batchNo],
        [t('production.order.page.operator') || '操作人员', order.operator || '--', t('production.order.page.status') || '订单状态', order.status === 'completed' ? (t('production.order.page.statusCompleted') || '已完成') : order.status === 'running' ? (t('production.order.page.statusRunning') || '生产中') : (t('production.order.page.statusPlanned') || '待生产')],
        [t('production.order.page.startTime') || '开始时间', order.startTime || '--', t('production.order.page.endTime') || '结束时间', order.endTime || order.estimatedEnd || '--']
      ]
      ...
      prodTitle.textContent = t('production.order.report.productionStats') || '生产统计'
      ...
      const prodStats = [
        { label: t('production.order.page.targetQty') || '目标数量', value: order.targetQty, unit: '瓶', color: '#909399' },
        { label: t('production.order.page.completedQty') || '完成数量', value: order.completedQty, unit: '瓶', color: '#409eff' },
        { label: t('production.order.page.qualifiedQty') || '合格数量', value: order.qualifiedQty, unit: '瓶', color: '#67c23a' },
        { label: t('production.order.page.unqualifiedQty') || '不合格数量', value: order.unqualifiedQty, unit: '瓶', color: '#f56c6c' },
        { label: t('production.order.page.qualifiedRate') || '合格率', value: order.qualifiedRate + '%', unit: '', color: '#67c23a' },
        { label: t('production.order.page.runtime') || '运行时长', value: order.runtime ? order.runtime + 'h' : '--', unit: '', color: '#e6a23c' },
        { label: t('production.order.page.alarmCount') || '报警次数', value: order.alarmCount, unit: '次', color: order.alarmCount > 0 ? '#f56c6c' : '#909399' },
        { label: t('production.order.page.downloadCount') || '已下载次数', value: includeDownloadCount ? (order.downloadCount || 0) : '--', unit: '次', color: '#909399' }
      ]
      ...
      if (includeAlarmDetail && order.alarmCount > 0) {
        ...
        alarmTitle.textContent = t('production.order.report.alarmDetail') || '报警明细'
      ...
      if (includeOperatorDetail) {
        ...
        opTitle.textContent = t('production.order.report.operatorDetail') || '操作人员明细'
      ...
      // 使用通用工具生成 PDF
      const filename = `${t('production.order.report.title') || '订单生产报告'}_${order.orderNo}`
