 ...
 *
 * 用法：
 * import { formatDate, parseDate, addDays, diffDays, isBetween } from '@/utils/data/date'
...
/**
 * 全局默认日期格式（从系统配置读取，支持实时更新）
 */
let globalDateFormat = getConfig('dateFormat', 'YYYY-MM-DD')

/**
 * 监听系统配置变化：日期格式改变时更新全局默认格式
 */
if (typeof window !== 'undefined') {
  window.addEventListener('dateFormatChanged', (event) => {
 ...
 * @param {String} format 格式化模板，默认使用全局默认日期格式 + 时间部分
...
export function formatDate(date, format) {
  ...
  if (!format) {
    format = globalDateFormat + ' HH:mm:ss'
  }
