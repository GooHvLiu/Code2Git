...
│   ├── api/                    # 接口
│   │   ├── user.js             # 登录、用户信息
│   │   ├── system/user.js      # 系统管理接口
│   │   └── biz/index.js        # 业务接口（治具/工位/设备/订单/生产/报表）
│   ├── assets/styles/          # 全局样式（variables、sidebar、transition...）
│   ├── components/             # 公共组件（SvgIcon 已就绪）
│   ├── business-components/    # 业务组件目录
│   ├── icons/svg/              # SVG 图标
│   ├── layout/                 # 后台布局（Sidebar + Navbar + TagsView + AppMain）
│   ├── router/index.js         # 路由（常量路由 + 动态路由，已配置所有菜单）
│   ├── store/modules/          # Vuex（app、user、permission、tagsView、settings）
│   ├── utils/                  # 工具（request、auth、validate、permission）
│   ├── filters/                # 全局过滤器
│   ├── permission.js           # 路由守卫（token 校验、动态路由、白名单）
│   ├── settings.js             # 系统配置
│   └── views/                  # 页面
│       ├── login/              # 登录页
│       ├── dashboard/          # 首页（含统计卡片、设备状态、订单进度）
│       ├── production/         # 生产执行（操作台、历史记录）
│       ├── order/              # 订单（列表、创建、详情）
│       ├── device/             # 设备（列表、监控）
│       ├── fixture/            # 治具（列表、注册）
│       ├── station/            # 工位
│       ├── report/             # 报表（生产、不良、OEE）
│       ├── system/             # 系统管理（用户、角色、菜单、字典、审计）
│       ├── kanban/             # 大屏看板（深色主题，独立路由）
│       └── error/              # 404、401
├── package.json
├── vue.config.js               # 端口 9528，别名 @
└── .env.development / .env.production
```

### 框架功能

✅ 完整的后台布局框架（侧边栏折叠、面包屑、标签页、响应式）

✅ 登录页 + 路由守卫 + Token 机制

✅ 动态路由权限（admin 角色可见所有菜单）

✅ 所有菜单已配置，点击可跳转

✅ 首页 Dashboard 有模拟数据展示

✅ 订单列表、创建设备列表、用户列表、审计日志等页面有示例数据

✅ 大屏看板页面（深色主题，访问 `/kanban` 免登录）

✅ WebSocket 客户端预留（`socket.io-client` 已安装）

✅ Axios 封装（统一错误处理、Token 自动携带）
