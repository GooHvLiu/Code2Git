  ...
  <div class="data-management-container">
    ...
    >
      <el-descriptions :column="2" border v-if="currentRow">
        ...
        <el-descriptions-item label="填充量"
          >{{ currentRow.fillVolume }} ml</el-descriptions-item
        >
        <el-descriptions-item label="生产状态">
          <el-tag size="small" :type="getStatusType(currentRow.status)">{{
            getStatusText(currentRow.status)
          }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="目标数量"
          >{{ currentRow.targetQty }} 瓶</el-descriptions-item
        >
        <el-descriptions-item label="已生产"
          >{{ currentRow.producedQty }} 瓶</el-descriptions-item
        >
        <el-descriptions-item label="合格数"
          >{{ currentRow.qualifiedQty }} 瓶</el-descriptions-item
        >
        <el-descriptions-item label="合格率"
          >{{ currentRow.qualifiedRate }}%</el-descriptions-item
        >
        <el-descriptions-item label="平均速度"
          >{{ currentRow.avgSpeed }} 瓶/h</el-descriptions-item
        >
        <el-descriptions-item label="开始时间">{{
          currentRow.startTime
        }}</el-descriptions-item>
        <el-descriptions-item label="结束时间" :span="2">{{
          currentRow.endTime || "生产中"
        }}</el-descriptions-item>
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    if (tab === "output") {
      ...
      data.push({
        ...
        startTime: timeStr,
        endTime: `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(
          date.getDate()
        )} ${pad(date.getHours() + 4)}:${pad(date.getMinutes())}:00`,
      });
    } else if (tab === "oee") {
      const plan = 8;
      ...
      const produced =
        status === "completed"
          ? target
          : Math.floor(target * (0.3 + Math.random() * 0.6));
      const qualified = Math.floor(produced * (0.95 + Math.random() * 0.049));
      data.push({
        id: i,
        batchNo: `B202608${pad(24 - Math.floor(i / 3))}${String(i).padStart(
          3,
          "0"
        )}`,
        productName: products[Math.floor(Math.random() * products.length)],
      ...
      data.push({
        ...
        endTime: `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(
          ...
          date.getMinutes() + Math.floor(duration / 60)
        )}:${pad(duration % 60)}`,
