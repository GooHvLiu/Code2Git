 ...
 *
 * 用法：
 * import { downloadFile, exportExcel, exportCsv } from '@/utils/business/export'
...
export async function downloadFile(url, params = {}, filename = '', method = 'get') {
  try {
    ...
    showSuccess(i18n.t('common.message.downloadSuccess'))
    return true
  } catch (error) {
    ...
    if (error?.data instanceof Blob) {
      ...
      reader.onload = () => {
        try {
          ...
          showError(i18n.t('common.message.downloadFailed'))
        } catch {
          showError(i18n.t('common.message.downloadFailed'))
        }
      }
