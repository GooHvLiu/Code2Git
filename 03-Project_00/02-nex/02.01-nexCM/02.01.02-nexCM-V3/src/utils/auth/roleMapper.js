...
export function getRoleName(role) {
  ...
  if (code) {
    // 优先从 i18n 获取内置角色名称
    const i18nName = i18n.t(`system.role.builtin.${code}.name`)
    if (i18nName && i18nName !== `system.role.builtin.${code}.name`) {
      return i18nName
    }
  }
...
export function getRoleDesc(role) {
  ...
  if (code) {
    // 优先从 i18n 获取内置角色描述
    const i18nDesc = i18n.t(`system.role.builtin.${code}.desc`)
    if (i18nDesc && i18nDesc !== `system.role.builtin.${code}.desc`) {
      return i18nDesc
    }
  }
