...
import {
  requestGetUserRouterMenuApi, requestTakenIsValidAPI
...
router.beforeEach(async (to, from, next) => {
  ...
  // 情况 2：有 token 并且 token 有效，即登录状态
  const isValid = await requestTakenIsValidAPI()
  // console.log("前端token是否有效：", isValid);
  if (isValid) {
    // token 有效，还要去登录页 → 直接跳首页，防止重复进入登录页
    if (to.path === "/login") {
      return next();
    }
    // 如果Vuex菜单为空，拉取菜单
