  ...
  <div class="permission-config-page">
    ...
    <div class="page-body">
      ...
      <div class="role-panel">
        <div class="panel-header">
          <span class="panel-title">
            ...
            {{ t('system.permission.page.roleList') }}
          </span>
          <el-tag size="small" type="info">{{ visibleRoles.length }}</el-tag>
        </div>
        <div v-loading="roleLoading" class="role-list" :element-loading-text="t('common.loading')">
          <div
          ...
          <div v-if="!roleLoading && visibleRoles.length === 0" class="empty-tip">
            ...
            <p class="empty-text">
              {{ t('system.permission.page.noRole') }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="permission-panel">
        ...
        <!-- 权限树形结构 -->
        <div v-loading="loading" class="permission-tree-wrapper" :element-loading-text="t('common.loading')">
          <div v-if="!currentRole" class="empty-tip empty-tip-large">
            <el-icon class="empty-icon"><Menu /></el-icon>
            <p class="empty-text">
              {{ t('system.permission.page.selectRoleTip') }}
            </p>
          </div>
          <el-tree
            v-else
        ...
        <!-- 底部统计和操作栏 -->
        <div v-if="currentRole" class="panel-footer">
          <div class="stats-info">
            <span class="stat-item">
              <el-icon><CircleCheckFilled /></el-icon>
              {{ t('system.permission.page.selected') }}:
              <b>{{ checkedCount }}</b>
            </span>
          ...
          <div class="footer-actions">
            <el-button size="small" :disabled="!hasChanges" @click="handleReset">
...
/**
 * 权限配置核心页面（permission-core）
 * 左右布局：左侧角色列表，右侧权限树勾选；可被系统设置 / 超级面板复用。
 * 作者：GooHv
 */
...
// ===== Props =====
const props = withDefaults(defineProps<PermissionConfigProps>(), {
  mode: 'system',
  showSuperAdmin: false,
  showSuperPermissions: false,
  titleKey: 'system.permission.page.title',
  descKey: 'system.permission.page.desc',
  saveSuccessKey: 'system.permission.page.saveSuccess'
})

// ===== 核心逻辑（permissionTree 同时作为 el-tree 的模板 ref） =====
const {
  loading,
  saving,
  roleLoading,
...
function getTypeName(type: string): string {
  const map: Record<string, string> = {
    menu: t('system.permission.page.menu'),
    button: t('system.permission.page.button'),
    param: t('system.permission.page.param')
  }
