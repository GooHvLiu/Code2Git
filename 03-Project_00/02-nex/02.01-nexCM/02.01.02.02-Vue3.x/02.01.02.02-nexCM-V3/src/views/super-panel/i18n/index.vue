  ...
  <div class="i18n-manager-container">
    ...
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <el-tooltip
          v-if="activeTab && activeTab !== masterLanguage && !translateManager.state.isTranslating"
            ...
            <div class="tree-container">
                  ...
                  <span class="custom-tree-node">
                    ...
                    <span
                      ...
                      :class="{
                        ...
                        'parent-match-node': isSearching && !data.isMatch && data.matchCount > 0
                      }"
                      >{{ data.label }}</span
                    >
                    <span v-if="!data.isLeaf && data.childCount !== undefined" class="tree-node-count"
                      >({{ data.childCount }})</span
                    >
                    <span v-if="isSearching && data.isMatch" class="match-badge">{{
                      t('superPanel.i18n.search.matchBadge')
                    }}</span>
                    <span v-if="isSearching && !data.isMatch && data.matchCount > 0" class="match-count-badge">{{
                      t('superPanel.i18n.search.matchCountBadge', { count: data.matchCount })
                    }}</span>
                  </span>
                </template>
              </el-tree>
              ...
              <div v-if="currentData && !loading" class="tree-footer">
                <span class="footer-stat"
                  ><el-icon><Folder /></el-icon> {{ totalFolderCount }} {{ t('superPanel.i18n.node.folders') }}</span
                >
                <span class="footer-stat"
                  ><el-icon><Document /></el-icon> {{ totalLeafCount }} {{ t('superPanel.i18n.node.items') }}</span
                >
                <span class="footer-stat footer-total"
                  ><el-icon><Menu /></el-icon> {{ totalNodeCount }} {{ t('superPanel.i18n.compare.total') }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="detail-panel">
            ...
            <div v-if="selectedIsLeaf" class="leaf-editor">
              ...
              <el-input
                ...
                type="textarea"
                :rows="3"
                :placeholder="t('superPanel.i18n.field.valuePlaceholder')"
                ...
                >
                  ...
                  <span class="item-actions">
                    ...
                    <el-dropdown
                      ...
                      trigger="hover"
                      placement="bottom"
                      @command="(format: string) => autoTranslateDetailItem(item, format)"
                    >
                      <el-button
                        type="text"
                        size="small"
    ...
    >
      <li @click="handleAddChild">
        <el-icon><Plus /></el-icon><span>{{ t('superPanel.i18n.node.addChild') }}</span>
      </li>
      <li @click="handleAddSibling">
        <el-icon><DocumentCopy /></el-icon><span>{{ t('superPanel.i18n.node.addSibling') }}</span>
      </li>
      <li class="divider"></li>
      <li
        v-if="contextMenu.data && !contextMenu.data.isLeaf"
      ...
      <li class="danger" @click="handleDeleteNode">
        <el-icon><Delete /></el-icon><span>{{ t('superPanel.i18n.node.deleteNode') }}</span>
      </li>
    </ul>
    ...
    >
        ...
        <el-form-item v-for="file in fileList" :key="file.langCode" :label="file.autonym">
          <div class="value-input-wrapper">
            <el-input
              ...
              :placeholder="t('superPanel.i18n.field.valuePlaceholderWithLang', { lang: file.autonym })"
            />
            <el-dropdown
              v-if="file.langCode !== masterLanguage && addDialog.form.values[masterLanguage]"
              trigger="hover"
              placement="bottom"
              @command="(format: string) => autoTranslate(file.langCode, format)"
            >
              <el-button
                size="small"
                type="primary"
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="addDialog.saving" @click="handleAddConfig">{{
          t('common.confirm')
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <!-- 备份路径设置对话框 -->
    <el-dialog
      v-model="backupPathDialog.visible"
            ...
            <el-table-column :label="t('common.operation')" width="120" align="center">
              <template #default="scope">
                <el-button
                  ...
                  :icon="Delete"
                  class="delete-btn"
                  @click="handleDeleteBackup((scope.row as any).fileName)"
                >
                  {{ t('common.delete') }}
                </el-button>
              </template>
    ...
    <!-- 批量翻译确认对话框 -->
    <el-dialog
      v-model="batchTranslateDialog.visible"
      :title="
        batchTranslateDialog.nodePath ? t('superPanel.i18n.translate.node') : t('superPanel.i18n.translate.batch')
      "
      width="500px"
    >
    ...
    <!-- 批量对比字段对话框 -->
    <el-dialog
      v-model="batchCompareDialog.visible"
    ...
    >
      <div class="batch-compare-config">
        <el-form :model="batchCompareDialog.form" label-width="160px">
          <el-form-item>
            ...
            <el-input
              :model-value="batchCompareDialog.masterLangName"
              readonly
          ...
          <el-form-item>
            ...
            <el-select
              ...
              :placeholder="t('superPanel.i18n.compare.pleaseSelectLanguage')"
              style="width: 300px"
            >
              <el-option
                v-for="file in fileList"
      ...
      <div v-if="batchCompareDialog.hasResult" class="batch-compare-stats">
        <div class="stat-card stat-blue">
          <div class="stat-value">{{ batchCompareDialog.stats.masterTotal }}</div>
          <div class="stat-label">{{ t('superPanel.i18n.compare.masterCount') }}</div>
        </div>
        <div class="stat-card stat-blue">
          <div class="stat-value">{{ batchCompareDialog.stats.targetTotal }}</div>
          <div class="stat-label">{{ t('superPanel.i18n.compare.targetCount') }}</div>
        </div>
        <div class="stat-card stat-orange">
          <div class="stat-value">{{ batchCompareDialog.stats.missingCount }}</div>
          <div class="stat-label">{{ t('superPanel.i18n.compare.missingCount') }}</div>
        </div>
        <div class="stat-card stat-green">
          <div class="stat-value">{{ batchCompareDialog.stats.extraCount }}</div>
          <div class="stat-label">{{ t('superPanel.i18n.compare.extraCount') }}</div>
        </div>
      </div>
      ...
      <div v-if="batchCompareDialog.hasResult" class="batch-compare-result">
        <div class="result-toolbar">
          <el-tabs v-model="batchCompareDialog.activeTab" size="small">
            <el-tab-pane
              :label="t('superPanel.i18n.compare.missingFields') + ' (' + batchCompareDialog.stats.missingCount + ')'"
              name="missing"
            />
            <el-tab-pane
              :label="t('superPanel.i18n.compare.extraFields') + ' (' + batchCompareDialog.stats.extraCount + ')'"
              name="extra"
            />
            <el-tab-pane
              :label="t('superPanel.i18n.compare.emptyFields') + ' (' + batchCompareDialog.stats.emptyCount + ')'"
              name="empty"
            />
          </el-tabs>
        ...
        <div class="result-table-wrapper">
          ...
          >
            ...
            <el-table-column :label="t('superPanel.i18n.compare.masterValue')" min-width="180" show-overflow-tooltip>
              <template #default="scope">
                <span v-if="(scope.row as any).masterValue !== null && (scope.row as any).masterValue !== undefined">{{
                  (scope.row as any).masterValue
                }}</span>
            ...
            <el-table-column :label="t('superPanel.i18n.compare.targetValue')" min-width="180" show-overflow-tooltip>
              <template #default="scope">
                <span
                  v-if="
                    (scope.row as any).targetValue !== null &&
                    (scope.row as any).targetValue !== undefined &&
                    (scope.row as any).targetValue !== ''
                  "
                  >{{ (scope.row as any).targetValue }}</span
                >
                <span v-else class="text-muted">-</span>
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="createLanguageDialog.visible" :title="t('superPanel.i18n.createLang.action')" width="550px">
      <el-form :model="createLanguageDialog.form" label-width="180px">
        ...
        <el-form-item>
          ...
          <el-select
            ...
            style="width: 100%"
            :placeholder="t('superPanel.i18n.createLang.codePlaceholder')"
          >
            <el-option
              v-for="lang in presetLanguages"
              :key="lang.code"
              :label="`${(lang as any)[languageDisplayField]} (${lang.code})`"
              :value="lang.code"
            >
        ...
        <el-form-item>
          ...
          <el-switch
            ...
            :active-text="t('superPanel.i18n.translate.copyValuesYes')"
            :inactive-text="t('superPanel.i18n.translate.copyValuesNo')"
          />
        </el-form-item>
        <el-form-item>
          <template #label>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="createLanguageDialog.saving" @click="handleCreateLanguage">{{
          t('common.confirm')
        }}</el-button>
      </template>
    </el-dialog>
...
const contextMenu = reactive<{ visible: boolean; x: number; y: number; data: any }>({
  visible: false,
  x: 0,
  y: 0,
  data: null
...
const addDialog = reactive<{
  ...
  mode: string
  parentPathArray: Array<string | number>
  form: { parentPath: string; key: string; values: Record<string, string> }
}>({
  visible: false,
  saving: false,
  translating: {},
...
const batchTranslateDialog = reactive<{ visible: boolean; mode: string; nodePath: string | null; nodeLabel: string }>({
  visible: false,
  mode: 'missing',
  nodePath: null,
  nodeLabel: ''
...
async function handleDeleteItem(item: TreeNode): Promise<void> {
  const ok = await confirmAction(`${t('superPanel.i18n.node.confirmDelete')}: ${item.keyPath}`, t('common.tip'), {
    type: 'warning'
  })
  if (!ok) return
  try {
...
function selectSearchResult(index: number): void {
  ...
  if (isSearching.value) {
    nextTick(() => {
      i18nTreeRef.value?.setCurrentKey(result.keyPath)
    })
  } else {
...
interface CompareResult {
  masterTotal: number
  targetTotal: number
  missingFields: CompareField[]
  extraFields: CompareField[]
  emptyFields: CompareField[]
}
function compareLanguageFiles(masterData: any, targetData: any): CompareResult {
  ...
  const missingFields = masterLeaves
    .filter(n => !targetKeySet.has(n.keyPath))
    .map(n => ({ keyPath: n.keyPath, masterValue: n.value, targetValue: null }))
  const extraFields = targetLeaves
    .filter(n => !masterKeySet.has(n.keyPath))
    .map(n => ({ keyPath: n.keyPath, masterValue: null, targetValue: n.value }))
...
function handleCopyReport(): void {
  ...
  fields.forEach((field: any, index: number) => {
    ...
    if (field.masterValue !== null && field.masterValue !== undefined)
      lines.push(`//   master: ${JSON.stringify(field.masterValue)}`)
    if (field.targetValue !== null && field.targetValue !== undefined && field.targetValue !== '')
      lines.push(`//   target: ${JSON.stringify(field.targetValue)}`)
...
async function confirmBatchTranslate(): Promise<void> {
  ...
  await batchTranslateLanguage(
    targetLangCode,
    targetLangName,
    batchTranslateDialog.mode,
...
async function handleCreateLanguage(): Promise<void> {
  ...
  if (!sourceLangCode) {
    showWarning(t('superPanel.i18n.translate.sourceNotEmpty'))
    return
  }
  if (!newLangCode) {
    showWarning(t('superPanel.i18n.createLang.codeNotEmpty'))
    return
  }
...
async function batchTranslateLanguage(
  ...
  try {
    ...
    try {
      ...
      const successMessage = t('superPanel.i18n.translate.batchSuccess', {
        lang: targetLangName,
        success: successCount,
        fail: failCount
      })
      if (translateManager.state.isCancelled) {
        const cancelMessage = t('superPanel.i18n.translate.cancelled', {
          lang: targetLangName,
...
async function handleDeleteNode(): Promise<void> {
  ...
  hideContextMenu()
  if (!data) return
  const ok = await confirmAction(`${t('superPanel.i18n.node.confirmDelete')}: ${data.keyPath}`, t('common.tip'), {
    type: 'warning'
  })
  if (!ok) return
  try {
...
async function handleAddConfig(): Promise<void> {
  ...
  if (!key) {
    showWarning(t('superPanel.i18n.field.keyNotEmpty'))
    return
  }
...
.language-empty-tip .tip-icon .el-icon {
  font-size: 40px;
  color: #409eff;
}
.tip-title {
...
.breadcrumb-bar :deep(.el-breadcrumb__inner.clickable) {
  cursor: pointer;
  color: #606266;
}
.breadcrumb-bar :deep(.el-breadcrumb__inner.clickable:hover) {
  color: #409eff;
}
.leaf-editor {
  padding: 20px;
