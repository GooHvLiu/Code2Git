        ...
        <div v-if="activeMenu === 'translation'" class="translation-config-panel">
          <div class="panel-header">
            <h3 class="panel-title">
              <el-icon><Connection /></el-icon>{{ t('superPanel.projectConfig.translation.menu') }}
            </h3>
          ...
          <div class="config-status-cards">
                ...
                <el-tag :type="translationConfig.enabled ? 'success' : 'info'" size="small">
                  {{
                    translationConfig.enabled
                      ? t('superPanel.config.translation.enabled')
                      : t('superPanel.config.translation.disabled')
                  }}
                </el-tag>
              </div>
            </div>
            <div class="status-card">
              ...
              <div class="status-value">
                <el-tag type="warning" size="small">{{ t('superPanel.config.translation.providerTencent') }}</el-tag>
              </div>
            </div>
            <div class="status-card">
              ...
              <div class="card-footer">
                ...
                <div class="config-actions">
                  ...
                  <el-tooltip
                    ...
                    :content="t('superPanel.projectConfig.tips.notInWhitelist')"
                    placement="top"
                  >
                  ...
                  <el-tooltip
                    ...
                    :content="t('superPanel.projectConfig.tips.codeConstant')"
                    placement="top"
                  >
    ...
    >
      <div class="editor-dialog-content">
        <div class="editor-toolbar">
          <div class="toolbar-left">
            ...
            <div class="backup-path-wrapper">
              ...
              <el-tooltip
                :content="editor.backupDirInfo.value.absolute || t('superPanel.projectConfig.editor.backupPathLoading')"
                placement="bottom"
              >
          ...
          <div class="toolbar-right">
            ...
            <el-button
              ...
              :disabled="!editor.editorDirty.value"
        ...
        <div class="editor-main">
          ...
          <div v-if="editor.showBackupPanel.value" class="backup-panel">
            <div class="panel-header">
              ...
              <el-button size="small" @click="editor.loadBackupList()"
                ><el-icon><Refresh /></el-icon
              ></el-button>
            </div>
            <div
              v-loading="editor.backupListLoading.value"
        ...
        <div v-if="editor.syntaxResult.value" class="syntax-result">
          <el-alert
            :title="
              editor.syntaxResult.value.valid
      ...
      <!-- 保存备注弹窗 -->
      <el-dialog
        v-model="editor.saveDialogVisible.value"
      ...
      >
        <el-form :model="editor.saveForm.value" label-width="80px">
          <el-form-item :label="t('superPanel.projectConfig.saveDialog.remark')">
            <el-input
              v-model="editor.saveForm.value.remark"
              type="textarea"
              :rows="3"
              :placeholder="t('superPanel.projectConfig.saveDialog.remarkPlaceholder')"
            />
          </el-form-item>
          <el-alert
        ...
        <template #footer>
          ...
          <el-button type="primary" :loading="editor.saving.value" @click="editor.confirmSave()">{{
      ...
      <!-- 修改备份路径弹窗 -->
      <el-dialog
        v-model="editor.backupPathDialogVisible.value"
      ...
      >
        <el-form :model="editor.backupPathForm.value" label-width="140px">
          ...
          <el-form-item :label="t('superPanel.projectConfig.backupPathDialog.newPath')">
            <el-input
              v-model="editor.backupPathForm.value.newPath"
              :placeholder="t('superPanel.projectConfig.backupPathDialog.newPathPlaceholder')"
            />
          </el-form-item>
          <el-alert
            :title="t('superPanel.projectConfig.backupPathDialog.warning')"
            type="warning"
            show-icon
            :closable="false"
          />
        ...
        <template #footer>
          ...
          <el-button
            type="primary"
            :loading="editor.changingBackupPath.value"
            @click="editor.confirmChangeBackupPath()"
          >
            {{ t('common.confirm') }}
          </el-button>
        </template>
...
function getEditTypeLabelKey(editType?: string): string {
  const map: Record<string, string> = {
    database: 'superPanel.projectConfig.editType.database',
    configFile: 'superPanel.projectConfig.editType.configFile',
    envFile: 'superPanel.projectConfig.editType.envFile',
    code: 'superPanel.projectConfig.editType.code'
  }
  return (editType && map[editType]) || 'superPanel.projectConfig.editType.code'
}
function getEditTypeTipKey(editType?: string): string {
  const map: Record<string, string> = {
    database: 'superPanel.projectConfig.editType.databaseTip',
    configFile: 'superPanel.projectConfig.editType.configFileTip',
    envFile: 'superPanel.projectConfig.editType.envFileTip',
    code: 'superPanel.projectConfig.editType.codeTip'
  }
...
function getEffectTypeLabelKey(effectType?: string): string {
  const map: Record<string, string> = {
    immediate: 'superPanel.projectConfig.effectType.immediate',
    restart: 'superPanel.projectConfig.effectType.restart',
    rebuild: 'superPanel.projectConfig.effectType.rebuild'
  }
...
function getEffectTypeTipKey(effectType?: string): string {
  const map: Record<string, string> = {
    immediate: 'superPanel.projectConfig.effectType.immediateTip',
    restart: 'superPanel.projectConfig.effectType.restartTip',
    rebuild: 'superPanel.projectConfig.effectType.rebuildTip'
  }
...
function getOwnerTypeConfig(ownerType: string) {
  return (
    (
      OWNER_TYPE_CONFIG as Record<
        string,
        { labelKey: string; descriptionKey: string; type: 'primary' | 'success'; icon: unknown }
      >
    )[ownerType] || OWNER_TYPE_CONFIG.backend
  )
}
...
async function openEditorFromCard(item: FlatItem): Promise<void> {
  ...
  if (!EDITABLE_WHITELIST.includes(filePath)) {
    const ok = await confirmAction(t('superPanel.projectConfig.tips.notInWhitelist'), t('common.warning'), {
      type: 'warning'
    })
    if (!ok) return
  }
