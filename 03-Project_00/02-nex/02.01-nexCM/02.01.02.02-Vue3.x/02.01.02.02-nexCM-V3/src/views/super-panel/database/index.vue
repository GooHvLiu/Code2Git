  ...
  <div class="database-manager">
                ...
                <div v-show="expandedCategories[categoryKey]" class="category-tables">
                  ...
                  >
                    ...
                    <div class="card-stats">
                      <span class="stat"
                        ><el-icon><DataLine /></el-icon>{{ table.table_rows || 0 }}
                        {{ t('superPanel.database.dataView.rows') }}</span
                      >
                      <span class="stat"
                        ><el-icon><Files /></el-icon>{{ formatFileSize(table.data_length || 0) }}</span
                      >
                    </div>
                    <div v-if="selectedTable === table.table_name" class="card-connector">
                      <el-icon><ArrowRight /></el-icon>
                    </div>
                  </div>
                </div>
            ...
            <div v-else class="data-content">
              <div class="data-header">
                  ...
                  <el-tag size="small" type="info" effect="plain">
                    {{
                      getCategoryName(
                        ...
                        currentTableInfo?.table_category || t('superPanel.database.categories.other')
                      )
                    }}
                  </el-tag>
                  <el-button size="small" type="primary" @click="loadTableData">{{
                ...
                <div class="data-search">
                  ...
                  />
                  <el-button size="small" type="primary" @click="loadTableData">{{
                    t('superPanel.database.page.search')
                  }}</el-button>
                </div>
              </div>
          ...
          <div class="config-list-panel">
            ...
            <div class="config-list">
              ...
              >
                ...
                <div v-if="editTableName === table.table_name" class="card-connector">
                  <el-icon><ArrowRight /></el-icon>
                </div>
              </div>
              <div v-if="configTables.length === 0" class="empty-tip">
                {{ t('superPanel.database.dataView.noConfigTable') }}
              </div>
            </div>
          </div>
              ...
              <div class="edit-content-header">
                <div class="edit-title">
                  <el-icon class="title-icon"
                    ><component :is="getTableIcon(currentEditTableInfo?.table_icon)"
                  /></el-icon>
                  <div class="title-text">
        ...
        <div class="tab-content backup-tab-content">
          <div class="backup-stats">
            <div class="stat-card">
              ...
              <div class="stat-info">
                ...
                <div class="stat-label">{{ t('superPanel.database.backup.total') }}</div>
              </div>
            </div>
            <div class="stat-card">
              ...
              <div class="stat-info">
                ...
                <div class="stat-label">{{ t('superPanel.database.backup.successCount') }}</div>
              </div>
            </div>
            <div class="stat-card">
              ...
              <div class="stat-info">
                ...
                <div class="stat-label">{{ t('superPanel.database.backup.failedCount') }}</div>
              </div>
            </div>
            <div class="stat-card">
              ...
              <div class="stat-info">
                ...
                <div class="stat-label">{{ t('superPanel.database.page.totalSize') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="backup-header">
            <div class="backup-actions">
              <el-button type="primary" size="small" @click="openBackupDialog">{{
            ...
            <div class="backup-tip">
              <el-icon><InfoFilled /></el-icon>{{ t('superPanel.database.backup.tip') }}
            </div>
          </div>
          ...
          >
            ...
            <el-table-column prop="backup_type" :label="t('superPanel.database.backup.type')" width="90" align="center">
              <template #default="scope">
                <el-tag v-if="scope.row.backup_type === 'full'" size="small" type="success">{{
            ...
            <el-table-column prop="status" :label="t('superPanel.database.page.status')" width="80" align="center">
              <template #default="scope">
                <el-tag v-if="scope.row.status === 'success'" size="small" type="success">{{
                  t('superPanel.database.page.success')
                }}</el-tag>
            ...
            <el-table-column :label="t('superPanel.database.page.operation')" width="150" fixed="right" align="center">
              <template #default="scope">
                <el-button
                  ...
                  @click="handleRestore(scope.row as BackupRow)"
                ...
                <el-button
                  ...
                  size="small"
                  style="color: var(--el-color-danger)"
                  @click="handleDeleteBackup(scope.row as BackupRow)"
                  >{{ t('superPanel.database.page.delete') }}</el-button
                >
              </template>
            </el-table-column>
        ...
        <div class="tab-content">
          <div class="restore-guide">
            ...
            <div class="guide-steps">
              <div class="step">
                ...
                <div class="step-content">
                  <h4>{{ t('superPanel.database.restore.step1Title') }}</h4>
              ...
              <div class="step">
                ...
                <div class="step-content">
                  <h4>{{ t('superPanel.database.restore.step2Title') }}</h4>
              ...
              <div class="step">
                ...
                <div class="step-content">
                  <h4>{{ t('superPanel.database.restore.step3Title') }}</h4>
              ...
              <div class="step">
                ...
                <div class="step-content">
                  <h4>{{ t('superPanel.database.restore.step4Title') }}</h4>
            ...
            <div class="restore-action">
              <el-button type="primary" @click="activeTab = 'backup'">{{
      ...
      <el-form :model="editForm" label-width="160px">
        <el-form-item v-for="col in editFormColumns" :key="col">
          ...
          <el-input
            ...
            :placeholder="t('superPanel.database.dataView.fieldSearchPlaceholder') + col"
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="backupCreating" @click="confirmCreateBackup">{{
        ...
        <el-form-item>
          <template #label>
            <span class="label-with-tip">
              ...
              <el-tooltip :content="t('superPanel.database.path.currentTip')" placement="top"
        ...
        <el-form-item>
          <template #label>
            <span class="label-with-tip">
              ...
              <el-tooltip :content="t('superPanel.database.path.newTip')" placement="top"
                ><el-icon class="label-tip-icon"><QuestionFilled /></el-icon
              ></el-tooltip>
            </span>
          </template>
        ...
        <el-form-item>
          <template #label>
            <span class="label-with-tip">
              ...
              <el-tooltip :content="t('superPanel.database.path.quickTip')" placement="top"
                ><el-icon><QuestionFilled /></el-icon
              ></el-tooltip>
            </span>
          </template>
...
async function loadAllTables(): Promise<void> {
  try {
    ...
    if (res.code === 200) {
      ...
      configTables.value = allTables.value.filter(
        tb => tb.table_name.includes('config') || tb.table_name.includes('feature')
      )
      await nextTick()
...
async function loadTableData(): Promise<void> {
  ...
  try {
    const res: any = await requestGetTableDataApi(
      selectedTable.value,
      dataPage.value,
      dataPageSize.value,
      dataSearch.value
...
function handleDataPageChange(page: number): void {
  dataPage.value = page
  loadTableData()
}
...
function handleEditSizeChange(size: number): void {
  editPageSize.value = size
  editPage.value = 1
  loadEditData()
}
function handleEditPageChange(page: number): void {
  editPage.value = page
  loadEditData()
}
...
async function saveEditRecord(): Promise<void> {
  try {
    if (editMode.value === 'add') {
      ...
      editColumns.value.forEach(col => {
        if (isPrimaryKey(col)) where[col] = editOriginalRow.value[col]
      })
...
async function handleDeleteRecord(row: Record<string, unknown>): Promise<void> {
  const ok = await confirmAction(
    t('superPanel.database.dataView.deleteConfirm'),
    t('superPanel.database.page.warning'),
    { type: 'warning' }
  )
  if (!ok) return
  try {
    const where: Record<string, unknown> = {}
    editColumns.value.forEach(col => {
      if (isPrimaryKey(col)) where[col] = row[col]
    })
...
async function confirmCreateBackup(): Promise<void> {
  ...
  try {
    const res: any = await requestCreateBackupApi(
      backupForm.backupType,
      backupForm.backupType === 'table' ? backupForm.tableName : null,
      backupForm.remark
    )
    if (res.code === 200) {
...
function handleBackupSizeChange(size: number): void {
  backupPageSize.value = size
  backupPage.value = 1
  loadBackupList()
}
function handleBackupPageChange(page: number): void {
...
async function handleRestore(backup: BackupRow): Promise<void> {
  const ok = await confirmAction(
    t('superPanel.database.restore.confirm', { name: backup.backup_name }),
    t('superPanel.database.page.warning'),
    { type: 'warning' }
  )
  if (!ok) return
...
async function handleDeleteBackup(backup: BackupRow): Promise<void> {
  const ok = await confirmAction(
    t('superPanel.database.backup.deleteConfirm', { name: backup.backup_name }),
    t('superPanel.database.page.warning'),
    { type: 'warning' }
  )
  if (!ok) return
...
.panel-header .el-input :deep(.el-input__inner) {
...
.table-stats-bar .stat-item .el-icon {
  font-size: 24px;
  color: #409eff;
  margin-right: 10px;
}
