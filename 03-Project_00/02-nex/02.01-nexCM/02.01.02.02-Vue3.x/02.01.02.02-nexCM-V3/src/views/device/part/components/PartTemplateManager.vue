  ...
  <div class="part-template-manager">
    <div class="page-toolbar">
      <div class="toolbar-left">
        <el-input v-model="searchKeyword" :placeholder="t('device.part.templateSearchPlaceholder')" clearable style="width: 280px" @clear="loadTemplateList" @keyup.enter="loadTemplateList">
          <template #append>
            <el-button v-permission="'device:part:template:search'" :icon="Search" @click="loadTemplateList"></el-button>
          </template>
        </el-input>
      </div>
      <div class="toolbar-right">
        <el-button v-permission="'device:part:template:add'" type="primary" :icon="Plus" @click="handleAdd">{{ t('device.part.templateAdd') }}</el-button>
        <el-button v-permission="'device:part:template:refresh'" :icon="Refresh" @click="loadTemplateList">{{ t('device.part.templateRefresh') }}</el-button>
      </div>
    </div>

    <el-table v-loading="loading" :data="filteredTemplates" :element-loading-text="t('common.loading')" border stripe style="width: 100%">
      <el-table-column :label="t('device.part.templateColumnTemplateName')" prop="name_key" min-width="150" align="center">
        <template #default="{ row }">
          <!-- row.icon 为后端字体类字符串，Vue3 下无法解析为组件，仅展示名称 -->
          {{ getTemplateName(row) }}
        </template>
      </el-table-column>
      <el-table-column :label="t('device.part.templateColumnTemplateKey')" prop="template_key" min-width="150" align="center" />
      <el-table-column :label="t('device.part.templateColumnCodePrefix')" prop="code_prefix" min-width="120" align="center" />
      <el-table-column :label="t('device.part.templateColumnDefaultSpec')" prop="default_spec" min-width="150" show-overflow-tooltip align="center" />
      <el-table-column :label="t('device.part.templateColumnDefaultRatedLife')" prop="default_rated_life" min-width="120" align="center">
        <template #default="{ row }">{{ row.default_rated_life }} {{ t('device.part.unitTimes') }}</template>
      </el-table-column>
      <el-table-column :label="t('device.part.templateColumnStatMethod')" prop="stat_method" min-width="140" align="center">
        <template #default="{ row }">{{ getStatMethodText(row.stat_method) }}</template>
      </el-table-column>
      <el-table-column :label="t('device.part.templateColumnStatTag')" prop="stat_tag" min-width="180" show-overflow-tooltip align="center" />
      ...
      <el-table-column :label="t('device.part.templateColumnAction')" width="180" align="center" fixed="right">
        <template #default="{ row }">
          <el-button
            v-permission="'device:part:template:edit'" type="primary" link size="small" :icon="Edit"
            :disabled="row.part_count > 0 || row.is_base_template === 1" @click="handleEdit(row)"
          >
            {{ t('common.edit') }}
          </el-button>
          <el-button
            v-permission="'device:part:template:delete'" type="danger" link size="small" :icon="Delete"
            :disabled="row.part_count > 0 || row.is_base_template === 1" @click="handleDelete(row)"
          >
            {{ t('common.delete') }}
          </el-button>
        </template>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? t('device.part.templateEdit') : t('device.part.templateAdd')" width="600px" :close-on-click-modal="false">
      <el-form ref="templateFormRef" :model="templateForm" :rules="templateRules" label-width="160px" class="template-dialog-form">
        <el-form-item prop="source_template_key">
          <template #label>
            {{ t('device.part.templateFormTemplateName') }}
            <el-tooltip :content="t('device.part.templateTipTemplateName')" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip>
          </template>
          <el-select v-model="templateForm.source_template_key" :placeholder="t('device.part.templateFormTemplateName')" style="width: 100%" :disabled="isEdit" @change="handleTemplateChange">
            <el-option v-for="item in sourceTemplates" :key="item.template_key" :label="getTemplateName(item)" :value="item.template_key ?? ''" />
          </el-select>
        </el-form-item>
        <el-form-item prop="default_spec">
          <template #label>
            {{ t('device.part.templateFormDefaultSpec') }}
            <el-tooltip :content="t('device.part.templateTipDefaultSpec')" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip>
          </template>
        ...
        <el-form-item prop="default_rated_life">
          <template #label>
            ...
            <el-tooltip :content="t('device.part.templateTipDefaultRatedLife')" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip>
          </template>
          <el-input-number v-model="templateForm.default_rated_life" :min="1" :max="9999999" :step="1000" controls-position="right" style="width: 100%" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {
  getPartTemplatesForAdmin, getBasePartTemplates, addPartTemplate, updatePartTemplate, deletePartTemplate
...
async function handleConfirm() {
  ...
  try {
    const data = isEdit.value
      ...
      : { source_template_key: templateForm.source_template_key, default_spec: templateForm.default_spec, default_rated_life: templateForm.default_rated_life }
    const res = isEdit.value && currentTemplate.value?.id != null
      ? await updatePartTemplate(String(currentTemplate.value.id), data)
      : await addPartTemplate(data)
    if (res.code === 200) {
      $msg.success(isEdit.value ? t('device.part.templateMessageEditSuccess') : t('device.part.templateMessageAddSuccess'))
...
<style scoped>
.part-template-manager { padding: 16px; }
...
.template-dialog-form :deep(.el-form-item__content) .el-input-number { width: 100%; }
</style>
