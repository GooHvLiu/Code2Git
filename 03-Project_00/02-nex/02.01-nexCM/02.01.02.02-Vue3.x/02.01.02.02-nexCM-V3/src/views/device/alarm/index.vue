          ...
          <el-row :gutter="12" class="charts-row">
              ...
              <div class="chart-panel">
                <div class="panel-header">
                  <span class="panel-title"
                    ><el-icon><PieChart /></el-icon>{{ t('device.alarm.categoryDist') }}</span
                  >
                </div>
            ...
            <el-col :span="10">
              <div class="chart-panel">
                <div class="panel-header">
                  <span class="panel-title"
                    ><el-icon><TrendCharts /></el-icon>{{ t('device.alarm.trend7d') }}</span
                  >
                </div>
                <div class="panel-body">
            ...
            <el-col :span="6">
              <div class="chart-panel">
                <div class="panel-header">
                  <span class="panel-title"
                    ><el-icon><DataAnalysis /></el-icon>{{ t('device.alarm.levelDist') }}</span
                  >
                </div>
                <div class="panel-body">
          ...
          <el-row :gutter="12" class="top-row">
            <el-col :span="24">
              <div class="chart-panel">
                <div class="panel-header">
                  <span class="panel-title"
                    ><el-icon><Rank /></el-icon>{{ t('device.alarm.top5Title') }}</span
                  >
        ...
        <div class="list-content">
            ...
            <el-form :inline="true" :model="searchForm" class="search-form">
              <el-form-item :label="t('device.alarm.colLevel')">
                <el-select
                  ...
                  :placeholder="t('device.alarm.all')"
                  clearable
                  size="small"
                  style="width: 120px"
                >
              ...
              <el-form-item :label="t('device.alarm.keyword')">
                <el-input
                  ...
                  :placeholder="t('device.alarm.keywordPlaceholder')"
                  clearable
                  size="small"
                  style="width: 180px"
                />
              ...
              <el-form-item>
                <el-button
                  ...
                  size="small"
                  @click="handleSearch"
                  >{{ t('device.alarm.search') }}</el-button
                >
                <el-button v-permission="'device:alarm:reset'" :icon="Refresh" size="small" @click="handleReset">{{
                  t('device.alarm.reset')
                }}</el-button>
              </el-form-item>
            </el-form>
          </div>
          ...
          <div class="toolbar-section">
            <div class="toolbar-left">
              <span class="total-text"
                >{{ t('device.alarm.totalPrefix') }} <b>{{ total }}</b> {{ t('device.alarm.totalSuffix') }}</span
              >
            ...
            <div class="toolbar-right">
              <el-button
                ...
                size="small"
                @click="handleExport"
                >{{ t('device.alarm.export') }}</el-button
              >
              <el-button v-permission="'device:alarm:refresh'" :icon="Refresh" size="small" @click="handleRefresh">{{
                t('device.alarm.refresh')
              }}</el-button>
            </div>
          </div>
          ...
          <div class="table-section">
            <el-table :data="pagedData" border stripe style="width: 100%" @selection-change="handleSelectionChange">
              ...
              <el-table-column
                prop="description"
                :label="t('device.alarm.colDescription')"
                min-width="200"
              ...
              <el-table-column
                prop="resolveTime"
                :label="t('device.alarm.colResolveTime')"
                width="160"
                align="center"
              />
              <el-table-column :label="t('device.alarm.colAction')" width="100" align="center" fixed="right">
                <template #default="{ row }">
                  <el-button
                    ...
                    link
                    size="small"
                    @click="handleDetail(row as AlarmRow)"
                    >{{ t('device.alarm.detail') }}</el-button
                  >
                  <el-button
                    ...
                    link
                    size="small"
                    @click="handleResolve(row as AlarmRow)"
                    >{{ t('device.alarm.handle') }}</el-button
                  >
                </template>
              </el-table-column>
            </el-table>
...
const trendLinePath = computed(() =>
  trendPoints.value.map((p, i) => (i === 0 ? `M${p.x},${p.y}` : `L${p.x},${p.y}`)).join(' ')
)
