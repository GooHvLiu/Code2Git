...
/** 分页查询角色列表 */
export function requestGetRoleListApi(params: Record<string, unknown>) {
  return request({ url: '/role/', method: 'get', params })
}

/** 获取角色详情（含菜单权限） */
export function requestGetRoleApi(id: string) {
  return request({ url: `/role/${id}`, method: 'get' })
}

/** 创建角色 */
export function requestCreateRoleApi(data: Record<string, unknown>) {
  return request({ url: '/role/', method: 'post', data })
}

/** 更新角色 */
export function requestUpdateRoleApi(id: string, data: Record<string, unknown>) {
  return request({ url: `/role/${id}`, method: 'put', data })
}

/** 删除角色 */
export function requestDeleteRoleApi(id: string) {
