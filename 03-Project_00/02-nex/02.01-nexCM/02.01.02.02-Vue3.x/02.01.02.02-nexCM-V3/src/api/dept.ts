...
/** 获取部门详情 */
export function requestGetDeptApi(id: string) {
  return request({ url: `/dept/${id}`, method: 'get' })
}

/** 创建部门 */
export function requestCreateDeptApi(data: Record<string, unknown>) {
  return request({ url: '/dept/', method: 'post', data })
}

/** 更新部门 */
export function requestUpdateDeptApi(id: string, data: Record<string, unknown>) {
  return request({ url: `/dept/${id}`, method: 'put', data })
}

/** 删除部门 */
export function requestDeleteDeptApi(id: string) {
