...
export default {
  ...
  'editType': {
    ...
    'code': 'Code Constant',
    'databaseTip': 'Stored in database, can be modified directly in System Settings/Super Panel config page, takes effect immediately',
    'configFileTip': 'Stored in backend config file (e.g. src/config/*.js), need to edit file online and restart backend service',
  ...
  'items': {
    'api': {
      ...
      'corsOrigin': {
        'description': 'Allowed cross-origin access source address, * means all sources allowed, specific domain is recommended for production',
        'label': 'CORS Origin'
      },
      'maxBodySize': {
    ...
    'database': {
      ...
      'waitForConnections': {
        'description': 'Whether to wait for connection release when pool is full, true means wait, false means immediate error',
    ...
    'storage': {
      ...
      'superPanel': {
        'license': {
          ...
          'timeGuardPath': {
            'description': 'Time guard file storage path, used to record last verification time, prevent system time rollback',
