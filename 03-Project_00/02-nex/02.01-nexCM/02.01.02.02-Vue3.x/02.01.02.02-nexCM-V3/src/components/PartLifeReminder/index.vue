    ...
    >
      ...
      <div class="reminder-header">
        ...
        <div class="header-right">
          <el-button text class="close-btn" :icon="Close" @click.stop="handleClose" />
        </div>
      </div>
...
defineOptions({ name: 'PartLifeReminder' })

/** 临期备件项 */
export interface PartLifeItem {
  name: string
...
function onDrag(e: MouseEvent): void {
  ...
  position.left = Math.max(0, Math.min(e.clientX - dragOffset.x, maxLeft))
  position.top = Math.max(0, Math.min(e.clientY - dragOffset.y, maxTop))
}
...
function getLifeClass(percent: number): string {
