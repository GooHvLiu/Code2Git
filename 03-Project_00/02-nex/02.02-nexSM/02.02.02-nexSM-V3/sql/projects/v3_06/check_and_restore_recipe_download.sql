...
-- 2. 如果下载按钮被删除了，就重新插入下载按钮
-- 注意：请先执行上面的查询，确认下载按钮是否被删除了
-- 如果下载按钮被删除了，请执行下面的INSERT语句

-- INSERT INTO nex_menu (id, parent_id, name, title, permission_code, type, sort, icon, path, component, visible, status, create_time, update_time)
-- VALUES ('_103_01_01', '_103_01', 'download', 'menu.production.recipe.download', 'production:recipe:download', 'button', 1, '', '', '', 1, 1, NOW(), NOW());

-- 3. 查询角色权限关联表中是否有下载按钮的关联记录
SELECT * FROM nex_role_menu WHERE menu_id = '_103_01_01';

-- 4. 如果角色权限关联表中没有下载按钮的关联记录，就为系统管理员角色添加下载按钮的关联记录
