  ...
  async kickDevice(req, res) {
    ...
    try {
      await notificationService.sendNotification({
        ...
        titleKey: 'notification.device.kicked.title',
        contentKey: 'notification.device.kicked.content',
    ...
    try {
      ...
      if (adminUserIds.length > 0) {
        await notificationService.sendBatchNotifications(adminUserIds, {
          titleKey: 'notification.device.kickedAdmin.title',
          contentKey: 'notification.device.kickedAdmin.content',
          contentParams: JSON.stringify({
            operator: operator.username,
            deviceName: device.device_name || '未知设备',
            userId: device.user_id,
