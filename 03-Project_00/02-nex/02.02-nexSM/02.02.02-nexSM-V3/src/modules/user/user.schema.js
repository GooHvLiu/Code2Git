...
/** 岗位类别 role varchar(50) DEFAULT 'operator'（动态从角色表验证，schema层只校验格式） */
const roleField = Joi.string().max(50).default(USER_ROLE.OPERATOR)
  .messages({
    'string.base': '岗位类别必须是字符串',
    'string.max': '岗位类别最大50个字符'
  });
