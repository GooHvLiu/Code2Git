...
**实现原理**
- 符合 GMP 21CFR Part 11 电子记录要求，自动记录关键操作的审计日志
- 后端：`src/modules/audit/` 模块，提供 `/audit/list`（全部）和 `/audit/my`（个人）接口
- 前端：`src/views/system/audit/index.vue` 页面，管理员看全部，普通用户只看自己
- 数据库表：`nex_audit_log`，记录操作人、操作类型、操作对象、修改前后值、结果、IP、时间
- 目前已接入：PLC 参数修改（写操作自动记录）

**权限控制**

| 角色 | 可见范围 | 操作权限 |
|------|---------|---------|
| 管理员（administrator） | 全部用户的审计日志 | 查看 + 按用户名/操作类型/时间范围筛选 |
