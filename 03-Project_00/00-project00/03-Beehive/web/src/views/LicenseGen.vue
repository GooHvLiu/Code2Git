      ...
      <el-col :span="16">
        <el-card>
          ...
          <el-form :model="form" :rules="rules" ref="form" label-width="110px" size="medium">
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="选择项目" prop="projectId">
                    <el-select v-model="form.projectId" placeholder="请选择项目" style="width:100%" @change="onProjectChange" filterable>
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="12">
                  <el-form-item label="最大用户数">
                    <el-input-number v-model="form.maxUsers" :min="0" :max="99999" style="width:100%" />
            ...
            <el-form-item>
              <el-button type="primary" size="medium" :loading="generating" @click="onGenerate" icon="el-icon-document-add">
                生成授权文件
              </el-button>
      ...
      <el-col :span="8">
        <el-card class="preview-card">
          ...
          <div class="preview-content">
            ...
            <div class="preview-item">
              <span class="preview-label">最大用户</span>
              <span class="preview-value">{{ form.maxUsers === 0 ? '不限制' : form.maxUsers + ' 人' }}</span>
            </div>
            <div class="preview-item">
    ...
    <el-dialog title="授权生成成功" :visible.sync="resultVisible" width="560px" custom-class="result-dialog">
      <div class="result-content">
        ...
        <el-descriptions :column="1" border size="small" style="margin-top:20px">
          ...
          <el-descriptions-item label="过期时间">{{ result.licenseData?.expiresAt ? formatTime(result.licenseData.expiresAt) : '永久有效' }}</el-descriptions-item>
        </el-descriptions>
      </div>
      <div slot="footer">
...
export default {
  ...
  data() {
    return {
      form: {
        projectId: '', projectName: '', licenseType: 'standard',
        validDays: 365, maxUsers: 0, machineId: '', features: [],
      ...
      projects: [], licenseTypes: [], featureOptions: [],
      bindMode: 'none', currentMachineId: '',
      generating: false, result: {}, resultVisible: false
    };
  },
  computed: {
    currentTypeLabel() {
  ...
  methods: {
    ...
    async onGenerate() {
      this.$refs.form.validate(async valid => {
        ...
        } catch (e) {} finally { this.generating = false; }
      });
    },
    onReset() {
...
.machine-info {
  ...
  margin-left: 110px; margin-bottom: 18px;
}

.preview-card { position: sticky; top: 20px; }
.preview-content {
  .preview-header { text-align: center; padding: 10px 0;
    .preview-icon { font-size: 48px; margin-bottom: 8px; }
    .preview-title { font-size: 18px; font-weight: 700; color: #1a1a2e; }
    .preview-sub { font-size: 12px; color: #909399; margin-top: 4px; }
  }
  .preview-item {
    display: flex; justify-content: space-between; align-items: center;
    padding: 10px 0; font-size: 13px;
    .preview-label { color: #909399; }
    .preview-value { color: #303133; font-weight: 500; max-width: 60%; text-align: right; word-break: break-all; }
  }
  .preview-tip {
...
.result-content { text-align: center; padding: 10px 0;
  .result-icon { font-size: 56px; margin-bottom: 12px; }
  .result-title { font-size: 20px; font-weight: 700; color: #1a1a2e; margin: 0; }
  .result-sub { font-size: 13px; color: #909399; margin-top: 6px; word-break: break-all; }
}
</style>
