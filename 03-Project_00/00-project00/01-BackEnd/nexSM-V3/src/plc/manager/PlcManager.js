...
class PlcManager {
   ...
   * @param {boolean} connected - 是否已连接（所有设备都连接才算连接）
   * @param {string} [deviceName] - 设备名称（可选，用于日志）
   */
  notifyPlcStatusChanged(connected, deviceName = null) {
    try {
      const message = {
        type: 'plc_status',
        data: {
      ...
      console.log(`[PLC] 连接状态已通知: ${connected ? '已连接' : '已断开'}${deviceName ? ` (设备: ${deviceName})` : ''}`)
    } catch (e) {
      console.error('[PLC] 通知连接状态失败:', e.message)
    }
  }
  ...
  async connectAll() {
    ...
    // 通知 PLC 连接状态变化
    this.notifyPlcStatusChanged(this.isAllConnected())
    return results
  }
  ...
  async disconnectAll() {
    ...
    // 通知 PLC 连接状态变化
    this.notifyPlcStatusChanged(false)
  }
