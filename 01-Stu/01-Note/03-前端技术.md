...
Bootstrap 是**免费、开源、移动端优先**的前端 UI 框架，内置大量 CSS/JS 样式类，无需手写大量 CSS，快速开发**响应式网页**。
...
官网下载压缩包（教程采用v3版本，最新版本v5），放入项目目录本地引入，无网络也可使用。

#### 通用规则

所有 Bootstrap 类名遵循：**功能 \+ 位置 \+ 尺寸 \+ 状态** 拼接规则，掌握规则 = 掌握 90% 类名。

- **m / p** → margin 外边距 / padding 内边距

- **t / b / l / r / x / y** → 上 / 下 / 左 / 右 / 左右 / 上下
...
```bash
mt-3  // margin-top 上外边距
mb-2  // margin-bottom 下外边距
mx-auto // 左右自动居中
p-3   // 四周内边距
px-4  // 左右内边距
py-2  // 上下内边距
...
容器布局分固定容器和流体容器，对应展示如下：

```html
<link
      rel="stylesheet"
      href="./sources/bootstrap-3.4.1-dist/css/bootstrap.min.css"
    />
    <style>
      .container-fluid {
        background-color: orange;
  ...
  <body>
    <div class="container-fluid">我是流体容器</div>
    <div class="container">我是固定容器</div>
    <script src="https://cdn.jsdelivr.net/npm/jquery@1.12.4/dist/jquery.min.js"></script>
    <script src="./sources/bootstrap-3.4.1-dist/js/bootstrap.min.js"></script>
```
...
> | 屏幕宽度              | 实际宽度 |
> | --------------------- | -------- |
> | ScreenWidth≥1200      | 1170     |
> | 1200＞ScreenWidth≥992 | 970      |
> | 992＞ScreenWidth≥768  | 750      |
> | 768＞ScreenWidth      | auto     |
...
<div class="container mt-5">
  <div class="row">
    <!-- 电脑端6列，手机12列通栏 -->
    <div class="col-md-6 bg-primary text-white p-3">左侧内容</div>
    <div class="col-md-6 bg-success text-white p-3">右侧内容</div>
  </div>
</div>
```
...
<div class="card p-3 shadow">
  <h5 class="card-title">用户卡片</h5>
  <p class="card-text">这是卡片内容区域</p>
</div>
```
...
> **核心类名说明**：
>
> - `navbar-expand-lg`：大屏展开、小屏折叠
>
> - `navbar-dark / navbar-light`：导航栏明暗主题
>
> - `collapse`：实现移动端折叠展开交互

##### 表格组件

Bootstrap 内置美化表格样式，支持条纹、悬浮、边框、紧凑样式，无需手写CSS，适配后台数据列表场景。
...
> **常用表格类名**：
>
> - `table-striped`：斑马条纹隔行变色
>
> - `table-hover`：鼠标悬浮高亮行
>
> - `table-bordered`：显示完整边框
>
> - `table-dark`：深色表头

##### 分页组件

标准后台分页组件，样式统一、自带选中状态，适配列表分页功能。
...
> **核心类名**：
>
> - `page-item active`：当前选中页码
>
> - `page-item disabled`：禁用不可点击页码
>
> - `justify-content-center`：分页居中展示

##### 面包屑

页面层级导航，清晰展示当前页面位置，常用于官网、后台详情页、多级页面。
...
Bootstrap5 支持自定义全局主题色，无需修改原源码，通过 CSS 变量覆盖默认样式：
...
- 统一间距、统一圆角、统一配色，页面极度整洁

- 适合快速开发后台管理系统、官网、展示型页面

- 不适合极度个性化 UI 设计（定制程度太高不建议）
...
弹窗、轮播、表单、分页、导航等复杂组件，**直接复制官方模板**，不需要背下来。

###### 真实流程

* 布局：container \+ row \+ col
* 排版：间距 m/p、对齐 text、弹性 d\-flex
* 组件：复制官方组件改内容
* 微调：阴影、圆角、颜色
...
<head>
  ...
  <body>
    <div id="root">
      ...
      <a v-bind:href="school.url.toUpperCase()" x="hello">我要去{{school.name}}学习百度1</a>
      <!-- v-bind可以简写为： -->
...
<!--- 或者简写为如下方式 --->
简单书写，单项数据绑定：<input type="text" :value="name" />
new Vue({
    el: '#root',
    data: {
      name: 'BeiJing'
...
<!--- 或者简写为如下方式 --->
简单书写，双项数据绑定：<input type="text" v-model="name"> 
new Vue({
    el: '#root',
    data: {
      name: 'BeiJing'
...
<body>
    ...
    <div id="root">
      ...
      <ul><li v-for="(p,index) of filPersons" :key="p.id">{{p.name}}-{{p.age}}-{{p.sex}}</li>
      </ul>
    </div>
    <script type="text/javascript">
      Vue.config.productionTip = false;
 ...
 */
      ...
      new Vue({
        ...
        data: {
          keyWord: "",
          persons: [ { id: "001", name: "马冬梅", age: 19, sex: "女" },{ id: "002", name: "周冬雨", age: 20, sex: "女" },{ id: "003", name: "周杰伦", age: 21, sex: "男" }, { id: "004", name: "温兆伦", age: 22, sex: "男" }] },
          computed: {
          filPersons() {
...
<body>
    ...
    <script type="text/javascript">
      ...
      new Vue({
        ...
        data: {
          keyWord: "",
          persons: [ { id: "001", name: "马冬梅", age: 30, sex: "女" }, { id: "002", name: "周冬雨", age: 31, sex: "女" },{ id: "003", name: "周杰伦", age: 18, sex: "男" },{ id: "004", name: "温兆伦", age: 19, sex: "男" }
          ],
...
<script type="text/javascript">
  ...
  new Vue({
    el: "#root",
    data: {userInfo: {account: "",password: "", age: 18,sex: "female",hobby: [],city: "beijing",other: "",agree: ""}},
