...
```bash
PS F:\CodingMan\Code2Git\01-Stu\02-SQL\02-MySQL> npm i mysql2

added 13 packages in 2s

3 packages are looking for funding
  run `npm fund` for details
PS F:\CodingMan\Code2Git\01-Stu\02-SQL\02-MySQL>
```

> mysql 8.0以上版本使用这个包，其他版本使用 npm i mysql

###### 配置模块

在使用 mysql 模块操作 MySQL 数据库之前，必须先对 mysql 模块进行必要的配置，主要的配置步骤如下：

operateMysql.js使用Promise，天然支持 await
...
insertOne("mydb4demo", "users", strObject);
/**
 * 插入单条数据到指定库.表
 * @param {string} dbName 数据库名
 * @param {string} tableName 表名
...
// Update更新指定的字段
{
  ...
  // 调用
  updateOne("mydb4demo", "users", updateObj, where);
  readAll("mydb4demo", "users", "*");
}
/**
 * 更新单条数据到指定库.表
...
将数据库的文件路径修改之后，记录当前路径和原始路径：

```文本
原始路径：F:\CodingMan\Code2Git\01-Stu\02-SQL\02-MySQL
当前路径：F:\CodingMan\Code2Git\01-Stu\06-SQL\02-MySQL
```
...
```ini
datadir=F:/CodingMan/Code2Git/01-Stu/06-SQL/02-MySQL/01-DataBase\Data
secure-file-priv="F:/CodingMan/Code2Git/01-Stu/06-SQL/02-MySQL/01-DataBase/Uploads"
```
...
> 执行成功提示：`[SC] DeleteService 成功`

###### 注册服务

```cmd
"C:\Program Files\MySQL\MySQL Server 9.7\bin\mysqld" --install MySQL97 --defaults-file="F:\CodingMan\Code2Git\01-Stu\06-SQL\02-MySQL\01-DataBase\my.ini"
```

> 重新注册服务，绑定正确路径的 my.ini
...
```cmd
"C:\Program Files\MySQL\MySQL Server 9.7\bin\mysqld" --defaults-file="F:\CodingMan\Code2Git\01-Stu\06-SQL\02-MySQL\01-DataBase\my.ini" --console
```

> 窗口不抛出 `secure-file-priv`、目录不存在报错，代表配置正常；
>
> 按 `Ctrl+C` 关闭控制台 mysqld 进程。
...
> 出现 `MySQL97 服务已经启动成功` 即完成。

##### 重要提示

若出现错误，可以通过如下命令进行错误收集查询：
...
```文本
原始路径：F:\CodingMan\Code2Git\01-Stu\02-SQL\01-MongoDB
当前路径：F:\CodingMan\Code2Git\01-Stu\06-SQL\01-MongoDB
```
...
systemLog:
  destination: file
  path: "F:\\CodingMan\\Code2Git\\01-Stu\\06-SQL\\01-MongoDB\\01-DataBase\\log\\mongod.log"
  logAppend: true
  verbosity: 0
storage:
  dbPath: "F:\\CodingMan\\Code2Git\\01-Stu\\06-SQL\\01-MongoDB\\01-DataBase\\data\\db"
  # 注意：MongoDB 6.1+ 不再需要 storage.journal.enabled 配置项
```
...
```cmd
"D:\Program Files\MongoDB\Server\bin\mongod.exe" --install --serviceName MyMongoDB --config "F:\CodingMan\Code2Git\01-Stu\06-SQL\01-MongoDB\01-DataBase\mongod.cfg"
```

> 重新注册服务，绑定正确路径的 mongod.cfg
...
```cmd
mongod --config "F:\CodingMan\Code2Git\01-Stu\06-SQL\01-MongoDB\01-DataBase_new\mongod.cfg"
```
