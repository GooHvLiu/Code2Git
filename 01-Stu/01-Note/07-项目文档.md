    ...
    async getCaptchaCode() {
      try {
        ...
        if (res.data.code === 200) {
          const svgText = res.data.data.img;
          this.captchaCade.src = `data:image/svg+xml;utf8,${encodeURIComponent(
            svgText
          )}`;
          this.captchaCade.uuid = res.data.data.uuid;
        } else {
...
`src/common/request/login.request.js`中只有一个请求函数如下：
...
`src/common/request/index.request.js`中只有一个校验函数如下：
