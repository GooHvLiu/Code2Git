...
<template>
  <div class="UserTags">
    <el-tag 
    size="small" class="Tag" closable :effect="item.isActive ? 'dark' : 'plain'"
...
以上的方法在刷新后，除了网站首页和激活状态的标签存在外，其他标签数据都丢失了，为了实现本地持久化保存，采用`localStorage`存储方式：

```js
// 在 methods 中创建持久化标签数组保存方法
saveTagsCache() {
...
因为用户信息会被很多组件共用，所以，一般情况下，用户信息都是会被`状态管理`模块所管理。
