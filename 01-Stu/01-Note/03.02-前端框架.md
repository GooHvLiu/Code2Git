...
Vue2+Vue CLI5 不需要`module-alias`，纯 webpack alias 即可实现路径简写，在vue.config.js中新增如下配置项：
...
* 模式1：默认原生 v-model，无需配置 model 选项

Vue2 内置默认规则，不需要写 `model{}`

```js
<ImportDialog v-model="visible" />
// 自动展开翻译
<ImportDialog :value="visible" @input="val => visible = val" />
```
...
* 模式 2：【自定义 v-model，需要配置 model 选项】

当你不想使用 `value` / `input` 这套命名（比如你的弹窗习惯用 `visible`），才需要增加 `model` 配置，用来修改 v-model 默认绑定的 prop 和事件
...
```js
<ImportDialog :visible="visible" @update:visible="val=>visible=val"/>
```
...
在实际使用中，`v-model`在`Vue2`中只能使用1次，但是如果需要多个变量双向绑定时，需要使用`变量.sync`方式实现:
...
> 以上即可实现`父组件的currentPage`和`子组件的page`双向绑定
...
> Vue 2 规定：在**自定义组件**上，`v-model` 默认等价于：
>
> ```html
> <!-- 你写的 -->
> <SearchBar v-model="keyword" />
> <!-- Vue 编译器自动展开成 -->
> <SearchBar :value="keyword" @input="keyword = $event" />
> ```
...
在此需要特别提示的是，在后续的学习中，学习到了全局API-基础全局-下轮执行，`Vue.nextTick`作为Vue的4个阶段，8个生命周期之外的第9个生命周期钩子，在此做简易介绍，如果想详细了解，请转至全局API-基础全局-下轮执行。
...
在此需要特别提示的是，在后续的学习中，学习到了路由-缓存路由-两个生命周期钩子，缓存路由的2个生命周期钩子作为Vue的4个阶段，8个生命周期之外的第10-11个生命周期钩子，在此做简易介绍，如果想详细了解，请转至FVue2-路由教程-常见配置-缓存路由-生命周期。

在缓存路由中，其仅被缓存组件触发的缓存专属生命周期有2个，分别是`activated()`和`deactivated()`，其作用是用于捕获路由组件的激活状态。

* `activated()`：组件激活（进入页面）

* `deactivated()`：组件失活（离开页面）

```js
export default {
  activated() {
    console.log('页面激活，刷新列表数据')
  },
  deactivated() {
    console.log('页面离开，清除定时器、取消请求')
  }
}
```
...
如下为在vue.config.js中增加devServer字段，可以新增额外配置项：
```js
const { defineConfig } = require("@vue/cli-service");
module.exports = defineConfig({
  transpileDependencies: true,
...
> 此配置可以配置端口号，是否打开浏览器和是否允许局域网访问本机项目
...
``是一套开箱即用、无需手写 CSS的 Vue 界面过渡组件库，原生同时支持 Vue2 / Vue3，API 几乎完全一致，是你从 Vue2 迁移 Vue3 最优选择。专门解决后台系统常见场景：弹窗、抽屉、下拉菜单、侧边栏折叠、面板展开收起、列表项显隐动画。

官方网址：https://github.com/MorevM/vue-transitions

###### 核心优势

1. Vue2 / Vue3 双版本支持，迁移成本极低
   - Vue2：`@morev/vue-transitions@2`
   - Vue3：`@morev/vue-transitions`（最新版）
2. 不需要手写 keyframes、transition 样式，组件式调用；
3. TransitionExpand 高度自动展开 / 折叠（核心亮点！Animate.css 做不到），完美适配侧边菜单、折叠面板；
4. 全部通过 props 控制时长、方向、缓动函数，统一规范动画；
5. 兼容 `v-if` / `v-show`，原生对标 Vue 内置`<Transition>`；
6. 支持 `TransitionGroup` 列表动画；
7. 动画风格克制，适合后台管理系统，不会出现过度花哨动画；
8. MIT 开源协议，无版权风险。
...
| 组件名称           | 作用                    | 后台常用场景                 |
| ------------------ | ----------------------- | ---------------------------- |
| `TransitionFade`   | 淡入淡出                | 消息提示、弹窗遮罩、简单面板 |
| `TransitionSlide`  | 方向滑动入场离场        | 弹窗、右侧抽屉、下拉菜单     |
| `TransitionScale`  | 缩放 + 淡入             | 弹窗、气泡卡片               |
| `TransitionExpand` | 高度 / 宽度自动展开折叠 | 侧边菜单、折叠面板、树形下拉 |

###### 基本安装

```bash
# Vue2 安装
npm install @morev/vue-transitions@2
# Vue3 安装
npm install @morev/vue-transitions
```

> Vue2版本号必须带上 @2，不带版本默认安装 Vue3 版本，直接报错

###### 局部方式

每个组件按需局部引入，减少打包体积，不会全局注册多余组件：
...
<template>
  <div>
    ...
    <!-- 滑动过渡 -->
    <TransitionSlide direction="down" :duration="280">
      <div v-if="visible" class="dialog">测试弹窗内容</div>
    </TransitionSlide>
  </div>
</template>
...
<script>
// 按需导入需要的组件
import { TransitionSlide, TransitionExpand, TransitionFade, TransitionScale } from '@morev/vue-transitions';
export default {
  components: {
    TransitionSlide,
    TransitionExpand,
...
在`main.js`中引入，适合追求开发便捷：

```js
import Vue from 'vue'
import VueTransitions from '@morev/vue-transitions'
// Vue2 不需要额外引入样式，内部自包含

Vue.use(VueTransitions, {
  // 全局统一默认配置（统一动画时长，规范项目）
...
| 参数       | 类型    | 默认       | 说明                                               |
| ---------- | ------- | ---------- | -------------------------------------------------- |
| `duration` | Number  | 300        | 动画时长，单位 ms；后台建议统一 `240~320`          |
| `easing`   | String  | `ease-out` | 缓动函数 `ease / ease-in / ease-out / ease-in-out` |
| `appear`   | Boolean | false      | 初始渲染时执行入场动画                             |
| `mode`     | String  | null       | 过渡模式，支持 `out-in / in-out`（切换组件使用）   |

###### 专属属性

1. `TransitionFade` 淡入淡出，基础淡入，适合遮罩、提示文字

```vue
<TransitionFade :duration="280">
  <div v-if="show">淡入内容</div>
</TransitionFade>
```

2. `TransitionSlide` 滑动动画（弹窗、抽屉高频使用）
...
<!-- 右侧抽屉 从右滑入 -->
<TransitionSlide direction="right" :duration="280">
  <div v-if="drawerVisible" class="right-drawer"></div>
</TransitionSlide>
```

3. `TransitionScale` 缩放动画（弹窗、气泡卡片）
...
```vue
<TransitionScale :scale="0.8" :duration="280">
  <div v-if="show">缩放弹窗</div>
</TransitionScale>
```

4. `TransitionExpand` 【侧边菜单 / 折叠面板】

   专属 props：`axis`

   - `y`：垂直高度展开（默认，折叠面板、下拉菜单）
   - `x`：水平宽度展开（侧边栏水平收缩）

```vue
<!-- 垂直展开：折叠面板、下拉菜单 -->
<TransitionExpand axis="y" :duration="320">
  <div v-if="open">折叠内容区域</div>
</TransitionExpand>

<!-- 水平展开：侧边栏水平收缩 -->
<TransitionExpand axis="x" :duration="320">
  <div v-if="sidebarOpen" class="sidebar-menu"></div>
</TransitionExpand>
```

> 重点：原生支持 `height:auto` 高度动画，不用手动写固定高度，原生 CSS 过渡很难实现

###### 组件事件

组件支持的事件，和 `Vue` 原生 `Transition` 一致。
...
> `enter`：入场动画开始；`after-enter`：入场动画结束；`leave`：离场动画开始；`after-leave`：离场动画结束

###### 列表动画

`TransitionGroup` 列表动画，表格新增行、菜单列表，用于 `v-for` 渲染列表，元素新增 / 删除带动画：

```vue
<TransitionFade tag="div" group>
...
在 Vue 2 开发中，当多个组件拥有相同的逻辑（如分页、表单校验、导出功能等）时，重复编写代码显然不够优雅。Mixin（混入）正是为了解决逻辑复用而生的。它本质上是一个普通的 JS 对象，里面可以包含 data、methods、生命周期钩子等组件选项。哪个组件需要用到这些功能，直接将这个对象“混入”到组件内部即可。

###### 定义文件

将项目中经常使用的模块提取出来`tablePage.mixin.js`
...
> 混入后，混入的变量和方法都成为了该组件自身的

###### 数据对象

组件和 Mixin 的 `data` 对象会进行递归合并。发生冲突时，以组件自身的数据优先：

```js
const myMixin = {
  data() { return { message: 'Hello from mixin', foo: 'abc' } }
}
const Component = {
  mixins: [myMixin],
  data() { return { message: 'Hello from component', bar: 'def' } }
}
// 最终组件的 data 为: 
// { message: 'Hello from component', foo: 'abc', bar: 'def' }
```

###### 值为对象

如 methods、components 等选项，将被合并为同一个对象。键名冲突时，取组件自身的键值对:

```js
const myMixin = {
  methods: { foo() { console.log('foo from mixin') } }
}
const Component = {
  mixins: [myMixin],
  methods: { foo() { console.log('foo from component') } }
}
// 最终调用 foo() 会输出: "foo from component"
```

###### 钩子函数

同名的钩子函数将被合并为一个数组，都会被调用。但执行顺序有严格规定：Mixin 的钩子总是先于组件自身的钩子执行:
...
1. 数据独立性：在多个组件中引入同一个 Mixin，各组件间的数据是完全不共享的。一个组件改动了 Mixin 里的数据，不会影响到其他组件。
2. 全局混入（慎用）：可以通过 `Vue.mixin()` 在 `main.js` 中全局注入。这会影响之后创建的所有 Vue 实例，极易污染全局环境，强烈不推荐。
3. 命名规范：建议使用描述性的名称，并以 `Mixin` 后缀结尾（如 `commonCardMixin`、`formValidationMixin`），便于识别。

4. 虽然 Mixin 提高了代码复用性，但在大型项目中存在明显的弊端：

   - 命名冲突：多个 Mixin 混入时，容易发生变量名隐式覆盖，导致难以排查的 Bug。

   - 来源不明：在模板中使用一个变量时，很难一眼看出它是来自哪个 Mixin，增加了维护成本（隐式依赖）。

5. Vue 2 最佳实践：

   - 每个 Mixin 应该专注于一个特定的功能，保持功能单一。

   - 为 Mixin 添加清晰的注释，说明其用途和依赖的方法。

   - 不要为了复用而过度使用 Mixin，避免创建过于复杂的“超级 Mixin”。
...
* <span style="color:orange">请注意，map方法是 Vue 组件模板 / 组件 script 内的语法糖（Vuex 辅助函数），只能在 `.vue` 单文件组件中使用；</span>
* <span style="color:orange">`store.commit()` 是原生 Vuex 底层 API，任何 js 文件，只要导入 store 实例，随时随地都能用；</span>
...
| 类型     | 原理                          | 特征                                           |
| -------- | ----------------------------- | ---------------------------------------------- |
| 后端路由 | URL 路径匹配服务器资源 / 接口 | 访问页面会刷新浏览器，服务器返回完整 HTML      |
| 前端路由 | URL 路径匹配 Vue 组件         | 仅修改浏览器地址栏，不发页面请求，局部切换组件 |

> 前端路由主要是为了实现SPA单页应用功能，即：整个项目仅一个 index.html，页面切换不刷新浏览器，通过 JS 替换页面组件，无整页网络请求，由前端路由控制页面切换。

##### Vue 路由

Vue 官方路由插件 vue-router，实现前端路由核心能力：
...
> 1. Vue2需要安装vue-router的V3版本，不可以直接安装最新版本，否则会报错
>
> 2. Vue3可以直接安装最新版本V4
...
单级路由也成本基本应用，是整个项目只有一个层级，每个路由规则下，不再有二级路由的简单应用。
...
import VueRouter from 'vue-router'
//引入组件
import UserAbout from '../views/UserAbout'
...
> 一级路由下的path后面的value必须加` / `

###### 路由命名

给路由配置`name`，跳转时不用写完整长路径，简化代码，一般情况下，单级路由是不需要路由命名的。
...
>
> ```vue
> <router-link class="list-group-item" active-class="active"  to="{name:'xiangguan'}">About</router-link>
> ```
>
> > 这在单级路由上的优势没有那么明显
...
在实际应用中，需要设置声明式导航（生成a标签，无页面刷新）和设置路由出口（匹配成功的组件渲染位置），对于使用路由页面来讲，需要执行：

* 不同的路由放置的位置和对应的样式设置（基本使用章节，主要以a标签为主），使用 `<router-link></router-link>`
* 不同的路由解析后放置的位置核对医用的样式设置，使用`<router-view></router-view>`
...
> 1. UserBanner.vue在components文件下，因不涉及路由相关概念，在此未做更细致的介绍
>
> 2. 实现切换`active-class可配置高亮样式`的使用：
>
> ```vue
> <router-link active-class="active" to="/about">About</router-link>
> ```
>
> 3. `to="/about"`,需要与路由index.js中路由path后面的一致
> 4. 指定展示位置
>
> ```vue
> <router-view></router-view>
...
* 路由组件通常存放在`pages`或者`views`文件夹，一般组件通常存放在```components```文件夹;
...
多级路由又成嵌套路由，是整个项目不只有一个层级，是相对于单级路由的复杂形式的别称，每个路由规则下，有且不仅仅只有二级路由的复杂应用。

##### 路由配置

###### 基本配置

更新路由配置文件 src/router/index.js
...
> 配置的二级路由需要在一级路由下通过去全新配置项`children`实现，但是其内部的path的value值不要再加` / `
...
>
> ```vue
> <router-link :to="{path: '/home/messages/detail',query: {id: m.id,title: m.title}}">{{ m.title }}
> </router-link>
> ```
>
> 使用路由命名之后的写法：
...
> > 1. 这在多级或者传参的基础上特别明显
> >
> > 2. 在params传参上必须使用name写法，不可以使用path写法

##### 全局挂载

在main.js中的全局挂载无需任何变化，和单级路由一致，没有任何变化。
...
创建两个全新的页面，分别为HomeMessages.vue和HomeNews.vue，其中HomeMessages.vue：
...
> 1. `<router-link class="list-group-item" active-class="active" to="/home/News">News</router-link>`下的to后面的value需要全路径value
> 2. 实际开发中，多级路由最多就是3层路由，不要嵌套过多的路由
...
1. URL 格式：`/detail?id=1&title=测试`
2. 不依赖命名路由，path /name 均可搭配
3. 刷新页面参数不丢失
...
* URL 格式：`/detail/1/Vue学习`
* 路由 path 必须提前声明占位符 `:key`
* 传参只能搭配 name 命名路由，搭配 path 参数会丢失
* 页面刷新参数丢失（缓存仅存在内存）

##### 固定传参

固定传参是指通过query传参，其基本使用分声明式传参 -->  组件接收参数 两个步骤。

###### 路由配置

修改/创建路由规则src/router/index.js：
...
声明式传参主要通过`:to=`来实现，HomeMessages.vue主要有字符串写法和对象写法：
...
> 推荐使用对象式传参，比较清晰，代码易读性强

###### 接收参数

在HomeMessagesDetail.vue中通过如下方式接收参数：
...
> 通过`$route.query.xxx`来获取对应对象中的数据
...
> 1. 需要特别注意的是，params传参必须将对应的路由设定name属性
>
> 2. 将需要传递的参数放在path路由后面作为占位符，本案例以`id`和`title`为例进行说明
...
> 推荐使用对象式传参，比较清晰，代码易读性强
>
> 需要特别注意的是，如果使用对象式传参，则不能使用path配置项，必须使用name配置
...
> 通过`$route.params.xxx`来获取对应对象中的数据

#### 常见配置

##### props配置

痛点：组件内大量书写`$route.params/$route.query`，耦合度高，维护麻烦，通过props的配置，可以减少书写重复代码。
...
* 函数写法：`props(route) {  return {    id: route.params.id,    title: route.query.title  } }`

  > 函数写法最为常用，兼容以上两种写法的全部功能，其本质是个回调函数

###### 对象写法

对象写法一般都是固定的对象值，在实际使用中比较少用。在src/router/index.js文件中：
...
> 通过props配置项获取对应参数，在展示数据的位置，可以减少代码的编写

###### 布尔写法

布尔写法只有params传递参数配套使用，其布尔值为真，就会把该路由组件收到的所有<strong style="color:orange">params</strong>参数，以props的形式传给Detail组件。在src/router/index.js文件中：
...
函数写法最常用，其本质是回调函数，兼容query/params传参，可以通过$route.query/params获取属性值，使用方法如下。在src/router/index.js文件中：
  ...
  children: [
    {
      ...
      path: "detail/:id/:title",
      component: HomeMessagesDetail,
      //props的第三种写法，值为函数,是个回调函数，可以通过$route.query/params获取属性值
      props($route) {
        return {
          id: $route.query.id,
...
```<router-link>```的replace属性，其作用是控制路由跳转时操作浏览器历史记录的模式

* 常规情况下，操作浏览器的前进后退动作，浏览器都会记录，并可以恢复；
* 浏览器的历史记录有两种写入方式：分别为```push```和```replace```，```push```是追加历史记录，```replace```是替换当前记录；
* 路由跳转时候默认为`push`；
* replace属性的设置就可以控制 操作浏览器 的记录是会否被记录，通最终导致是否可以恢复；

###### 常规方式

replace是归属于`<router-link>`的属性标签，其本质是一个布尔值，控制浏览器的历史记录的写入方式，

```vue
<!-- 跳转路由并携带params参数，to的对象写法 -->
<router-link replace :to="{ name: 'xiangqing', params: { id: m.id, title: m.title }  }" >
  {{ m.title }}
</router-link>
```
...
路由切换默认销毁组件实例，再次进入页面会重新发请求、重置表单；`keep-alive`缓存组件实例，组件不会销毁，数据 / 表单状态保留。可以确保组件内的数据持续保留，避免被销毁重置，重新填写。
...
HomeNews.vue中添加过度，希望在切走之后暂停，切回来继续：
...
> 使用缓存路由的生命周期钩子，可以有效利用资源，在不使用的时候暂停消耗内存

###### 基础使用

只需要在需要保留的<router-view></router-view>外层包裹一个<keep-alive></keep-alive>标签：

* 若想要其`<router-view></router-view>`包裹的<strong style="color:orange">所有vue组件都保留数据，则不需要填写其他配置项</strong>；
* 若想要其`<router-view></router-view>`包裹的<strong style="color:orange">指定一个Vue组件保留数据，可以通过`include="组件名"`进行设定</strong>；
* 若想要其`<router-view></router-view>`包裹的<strong style="color:orange">指定多个Vue组件保留数据，可以通过`:include="['组件名1','组件名2']"`进行设定</strong>；
* 若想要其`<router-view></router-view>`包裹的<strong style="color:orange">指定Vue组件被排除在外，可以通过`exclude="组件名"`进行设定</strong>；
...
在HomeNews.vue中添加输入框，用于测试数据是否被保留：
...
> 后续查看input框内的输入数据是否在切换之后被删除

在UserHome.vue中修改` <router-view></router-view>`标签如下，即可实现`HomeNews.vue`内input标签的数据保留：
...
<template>
  <div>
    ...
    <div>
      ...
      <div>
        <!-- 缓存多个路由组件 -->
        <keep-alive include="['HomeNews','HomeMessages']">
          <router-view></router-view>
        </keep-alive>
      </div>
    </div>
...
* hash模式，是指前端在请求链接后会增加一个`/#/`，其后的任何字符串都不会传递给后端服务器
* history模式，是指前端会将请求链接全部传递给后端服务器
  * 在实际工作当中，因为前端存在前端路由，整个前端的请求网址会发生内部路由参数；
  * 如果使用history模式后，在项目正式上线后，会导致向后端的请求链接与实际不符；
  * 一般这种情况会通过后端的设置或者使用插件【connect-history-api-fallback】实现规避这个问题
...
在src/route/index.js文件的路由创建内增加字段：
...
> `mode: "history",`或者`mode: "hash",`，默认为hash模式，所以，如果是hash模式，可以不写

###### 上线优化

从表面上，hash模式是最佳选择，但是由于history模式具有较好的美观性和审核合规性，很多时候，还是会选择使用history模式，那么上线后报404错误如何处理呢？

前端一旦打包上线，history模式就只能通过后端程序员解决了，本章节采用node+express方式部署后端，通过如下方式实现优化掉404错误问题：

* 安装node依赖包`npm i connect-history-api-fallback`;
...
* 安装使用：
  ...
  const app express()
  // 使用依赖包（必须在使用静态资源之前；本身是个函数，必须history()使用）
  app.use(history())
...
匹配不到路由自动跳转 404 页面，`*`通配符必须放在路由数组最后：
...
重复跳转报错修复 NavigationDuplicated:多次点击同一路由控制台抛出错误，重写 push/replace 捕获异常

```js
// router/index.js 文件末尾添加
...
根据用户角色，页面初始化时动态注册路由，动态添加路由（如后端权限菜单）：
...
后台标准 Layout 嵌套布局:侧边栏 + 顶部导航 + 子页面通用布局
...
>  Layout.vue 包含侧边栏、顶部导航、子路由`<router-view>`出口
...
// 3 动态参数重定向
{ path: '/', redirect: to => ({ name: 'Detail', params: { id: to.params.id } }) }
```
...
全称为编程式路由导航，也就是不用`<router-link>`通过 JS 代码控制页面跳转，按钮、异步请求后跳转场景使用。其实现的底层原理就是通过`this.$router.push()`和`this.$router.replace()`两个API实现。

##### 基本实现

###### 新增历史

使用 `this.$router.push()` 新增历史
...
在对应需要增加路由显示的页面进行优化代码，本次以`HomeMessagesDetail.vue`为例，创建两个按钮和对应的方法pushShow和repalceShow:
...
> 如上为了验证`this.$router.push()`和`this.$router.replace()`的两个方法

在src/components/UserBanner.vue中修改如下：
...
> 如上为了验证`this.$router.back()`、`this.$router.forward()`和`this.$router.go()`的三个方法

#### 路由守卫

##### 基本概念

在路由配置中设置，其作用是对路由进行权限控制，守卫的本质是路由跳转前后的拦截函数，用于登录校验、页面标题、权限判断。
...
路由守卫实际上是路由器身上的函数，所以，不能直接暴露路由器，而是需要创建路由变量，并继续函数创建。
...
> 1. 在index.js的最后需要进行暴露`export default router;`
>
> 2. 每个路由配置中的meta为路由元，意思为程序员可以进行自定义变量的对象，在此主要就是为了：
>    1. 每次切换，通过后置守卫对网页主题显示内容进行修改设置
>    2. 增加`isAuth`的布尔变量，用于权限判断进行路由守卫
...
> 通过`to.meta.isAuth`的布尔判断是否需要鉴权，如果需要鉴权，进入鉴权判断
...
独享守卫是在路由配置中设置的，是指定路由配置项下设置的，内部代码和全局前置守卫一样应用。
...
| 语法                            | 用途                              |
| ------------------------------- | --------------------------------- |
| `<router-link to>`              | 声明式页面导航                    |
| `<router-view>`                 | 路由组件渲染出口                  |
| `$router.push/replace/back/go`  | JS 编程式导航                     |
| `$route.query`                  | 获取 url 查询参数？xxx=xx         |
| `$route.params`                 | 获取路径动态参数 /:id             |
| `props`                         | 路由参数解耦，组件直接 props 接收 |
| `router.beforeEach`             | 全局前置守卫（登录拦截）          |
| `router.afterEach`              | 全局后置守卫（修改页面标题）      |
| `beforeEnter`                   | 单路由独享守卫                    |
| `beforeRouteEnter/Update/Leave` | 组件内路由守卫                    |
| `$route.meta`                   | 读取路由自定义元信息              |
| `mode: hash / history`          | 浏览器地址栏模式                  |
| `()=>import()`                  | 路由懒加载，分包优化              |
| `scrollBehavior`                | 控制页面滚动位置                  |
| `router.addRoutes()`            | 动态添加权限路由                  |
| `<keep-alive>`                  | 缓存路由组件，保留页面状态        |
| `activated / deactivated`       | 缓存组件专属生命周期              |
...
```文本
src/router/
├── index.js               # 路由入口、守卫、模式、滚动行为
├── permission.js          # 路由权限守卫逻辑抽离
└── modules/               # 按业务拆分路由模块
    ├── user.js
    ├── order.js
    └── system.js
...
使用webpack技术，自研vue-cli，官方文档：https://cli.vuejs.org/zh/guide/creating-a-project.html#vue-create
...
使用vite，官方文档：https://v3.cn.vuejs.org/guide/installation.html#vite
...
![image-20260913170249745](./01-MD/Images/image-20260913170249745.png)
...
针对 App.vue内 template 内无需根目录的报错:

```text
用 Vue2 的插件 Vetur，使用 Vue3 的插件 Volar，消除vue文件中 template 下没有根节点问题；
```

针对测试研发项目，建议屏蔽语法检测功能，修改vue.config.js：
...
我们主要针对使用 vue-cli 工具生成的vue3模板，主要对main.js和app.vue的工程结构与vue2时期的相同实现做横向对比。

#### 分析结论

##### 语法检查

为了更好地调试程序和验证，可以通过如下方式关闭语法检查：
...
这里指代的是 vue3 中的 ref 函数。 

###### 基本概念

ref函数是 Vue3 的新的配置项，用于响应式的数据：
...
<template>
  ...
  <h2>姓名：{{ name }}</h2>
  <h2>年龄：{{ age }}</h2>
  <button @click="changeInfo">数据响应</button>
</template>
<script>
import { ref } from "vue";
export default {
  name: "App",
  // 此处只是测试一些setup,暂时不考虑响应式的问题。
  setup() {
    // 数据
    let name = ref("张三");
    let age = ref(18);
...
> 基本数据类型的 Ref 引用，响应靠``Object.defineProperty()``的```get```与```set```完成的
...
> 对象类型的数据，响应靠内部 <i style="color:gray;font-weight:bold">“ 求助 ”</i> 了Vue3.0中的一个新函数—— ```reactive```函数，实际就是 proxy 代理实现

##### 对象类型

这里指代的是 vue3 中的 reactive ：
...
> 可以全部通过 reactive 函数实现，将变量转换为对象实现
   ...
   2. <span style="color:#aad">若返回一个渲染函数：则可以自定义渲染内容。（了解）</span>
      ...
      export default {
        ...
        setup() {
          ...
          // 返回值，方式二：返回渲染函数
          return () => h("h1", "我是 setup 返回的渲染函数");
        }
      };
      </script>
      ```

      > 若 setup 返回是一个渲染函数，则页面结构设计失效，全部以渲染函数为主
...
> watch监控的是ref结构，而不是实际的值，所以，不能使用
...
* 左边是Vue 2.x 的生命周期；右边是Vue 3.x 的生命周期；
...
<img src="./01-MD/Images/2-1789374631701-1.png" alt="Vue2.x" style="zoom:35%;" /><img src="./01-MD/Images/3.png" alt="Vue3.x" style="zoom:35%;" />
...
在Demo.vue中引入使用hooks/usePoint.js文件：
...
> 一般应用于：当从其他地方获取的数据，为了防止不小心更改了，将数据设置为`readOnly`，可以防止被修改
...
![image-20260914184402071](./01-MD/Images/image-20260914184402071.png)
...
<img src="./01-MD/Images/image-20260914190523542.png" alt="image-20260914190523542" style="zoom:50%;" />
...
本章节主要介绍`Vue2+Element UI`实现文件的上传功能。
...
- 分支B（云端存储）：使用 `multer.memoryStorage` 将文件读入内存，通过`AWS SDK`直传至`Cloudflare R2`;

#### 创建项目

##### 生成项目

通过`vue2`的脚手架工具创建项目文件夹：

```bash 
PS F:\CodingMan\Code2Git\01-Stu\03-FrontEnd\04-Tec_Support> vue create 01-upload-files
```

##### 项目模板

改造`HelloWorld.vue`作为测试上传文件机床：

```vue
<template>
  <div class="hello">
    <h1>Uploading Files Tool Page</h1>
  </div>
</template>
...
```bash
PS F:\CodingMan\Code2Git\01-Stu\03-FrontEnd\04-Tec_Support\01-upload-files> npm i element-ui -S
...
```bash
PS F:\CodingMan\Code2Git\01-Stu\03-FrontEnd\04-Tec_Support\01-upload-files> npm install babel-plugin-component -D
...
在`UI库`中获取对应模块并引入`main.js`，在`HelloWorld.vue`中使用。
...
> 在`element`上获取：https://element.eleme.cn/2.13/#/zh-CN/component/upload#methods

##### 引入模块

在`main.js`中引入`upload`：
...
export default {
  ...
  methods: {
    ...
    async submitUpload() {
      ...
      try {
        // 假设后端运行在 3000 端口 api 接口为api/upload
...
> 1. 网络请求采用`axios`，安装依赖：`npm install axios@1.6.8`;
> 2. `axios`文件上传采用`post`请求
...
将`src/components/HelloWorld.vue`界面下载增加：
...
> 下载文件时，不要采用axios请求使用原生`Windows.open(`http://localhost:3000/api/download/${index}`, "_blank")`
