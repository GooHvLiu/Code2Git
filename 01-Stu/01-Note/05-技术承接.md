...
如下为01-Promise\_结构搭建.js代码-主要体现构造函数部分
...
基本目标：学会在 Express 项目集成接口文档，自己写注释自动生成网页文档，支持在线调试；后面再套入项目实战。 

技术栈：`express` + `swagger-autogen` + `swagger-ui-express` 文档页面：

- SwaggerUI：可在线调试接口
- swagger-autogen：自动扫描路由

### 基本概念

1. **OpenAPI**：接口描述规范（原来叫 Swagger），用来描述接口地址、请求参数、返回数据。
2. **swagger-ui-express**：把规范渲染成网页，提供 Try it out 在线调试。
3. **swagger-autogen**：自动扫描`router.get / router.post` 代码，自动提取请求方法、接口路径、路径参数（`/:xxx`），自动生成 OpenAPI 文档，搭配 swagger-ui-express，页面自带 Try it out 在线调试。

> - 可以识别：GET POST PUT DELETE、完整 url、路径参数 `/api/hosp/:id`
> - 无法自动猜出来：query 参数、请求 body、返回 data 结构（这部分可以后面手动补充，不影响接口先展示出来）
...
```bash
npm install express cors swagger-autogen swagger-ui-express
```

> - express：web 服务
> - cors：跨域（前端 Vue 访问后端必备）
> - swagger-ui-express：swagger 网页 UI
> - swagger-autogen：自动扫描工具

#### 基础设施

##### 生产配置

新建文件 `openAPI/swagger.js`（自动生成 spec 配置）:
...
搭建基础 express + swagger 主文件 `app.js`：
...
// ========== 路由（你现有的尚医通路由，**完全不用改路由代码，不用加注释！**） ==========
const hospRouter = require('./routes/hosp');
app.use('/api/hosp', hospRouter);
...
```bash
node ./openAPI/swagger.js
```

> 1. 这一步会读取 app.js 里面所有挂载的 router，自动抓取全部接口路径，不需要在路由文件写任何注释；
>
> 2. 项目openAPI/data下会自动生成 `swagger-output.json`，它就是扫描所有路由生成的 OpenAPI 规范文件。

##### 默认自启

修改`package.json`文件，可以实现项目启动前自生成接口文件：

```json
"scripts": {
    "start": "nodemon app.js",
    "gen-swagger": "node ./openAPI/swagger.js",
    "dev": "npm run gen-swagger && nodemon --watch app.js"
  },
```
...
- SwaggerUI：`http://localhost:3000/api-docs` 操作：点开接口 → `Try it out` → 填参数 → `Execute`，直接看返回结果。

### 核心优点

1. 零注释，不用写一大段 YAML，直接复用你已经写好的 router 代码，适合尚医通 Mock 项目快速搭建文档
2. 自动识别路径参数，例如`router.get('/findByHoscode/:hoscode')`会自动识别`hoscode`路径参数
3. 保留 swagger 原生页面，支持 Try it out 在线调试接口

### 局限缺陷

1. 无法自动识别 query 参数：`/api/hosp/list?page=1&limit=10` 的 page、limit 它识别不到，需要手动在 swagger-output.json 补充，或者写少量提示注释
2. 不能自动识别 post 的请求 body 和返回 data 结构。只能拿到接口地址，字段描述需要手动补充。

> 适合你的场景：学习前端，只需要接口路径存在，能在线请求 mock 数据，不需要详细字段描述，首选这个方案

### 项目实战

该项目实现基于`尚医通`案例实现，把这套方案迁移到尚医通 Mock 后端。
